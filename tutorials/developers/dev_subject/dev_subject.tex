...
We will start by implementing in detail \code{SubjectCON}. This class can be used for subjects with connectivity matrix data usually obtained from DTI.
...
for i = 1:1:50 ¥\circled{6}\circlednote{6}{iterates to create 50 subjects.}¥
    sub = SubjectCON( ... ¥\circled{7}\circlednote{7}{creates a new subject of the class \code{SubjectCON}. For that, it defines the \code{'ID'}, \code{'LABEL'}, \code{'NOTES'}, \code{'BA'} (Brain Atlas) and \code{'CON'} (a random adjacency matrix) for the new subject.}¥
    ...
    gr.get('SUB_DICT').get('ADD', sub) ¥\circled{10}\circlednote{10}{adds the created subject \code{'sub'} into group of subjects.}¥
end
...
¤%%% ¡prop! 
L (data, scalar) is the number of layers of subject data. ¥\circled{1}\circlednote{1}{defines a parameter to determine the number of layers of subject data. This property must be a scalar parameter.}¥
%%%% ¡default!
2¤ ¥\circled{2}\circlednote{2}{defines the default option, in this case \code{'2'}.}¥

¤%%% ¡prop! 
LAYERLABELS (metadata, stringlist) are the layer labels provided by the user.¤ ¥\circled{3}\circlednote{3}{defines a parameter to determine the labels for each layer. This property must be a string list parameter.}¥
...
check = (iscell(value) && isequal(length(value), num_layers)  && isequal( cellfun(@(v) size(v, 1), value), ones(1, num_layers) * br_number)  && isequal( cellfun(@(v) size(v, 2), value), ones(1, num_layers) * br_number)) || (isempty(value) && br_number == 0);¤ ¥\circled{6}\circlednote{6}{checks that the size of each layer is equal to the number of brain regions.}¥
if check
    msg = 'All ok!';
else   
...
for i = 1:1:10 
    sub = SubjectCON_MP( ...
        ...
        ); ¥\circled{3}\circlednote{3}{constructs 3 layers randomly of the same size as the number of brain regions.}¥
...
for i = 1:1:50
    sub = ¤SubjectFUN¤( ...
        ...
        'BA', ba, ...
        ¤'FUN', rand(10, ba.get('BR_DICT').get('LENGTH')) ...¥\circled{1}\circlednote{1}{constructs the random adjacency matrix with a size of 10 timepoints by the number of brain regions.}¥¤
        );
    sub.memorize('VOI_DICT').get('ADD', VOINumeric('ID', 'Age', 'V', 100 * rand()))
    sub.memorize('VOI_DICT').get('ADD', VOICategoric('ID', 'Sex', 'CATEGORIES', {'Female', 'Male'}, 'V', randi(2, 1)))
    gr.get('SUB_DICT').get('ADD', sub)
...
check = (iscell(value) && isequal(length(value), num_layers)  && isequal( cellfun(@(v) size(v, 2), value), ones(1, num_layers) * br_number)) || (isempty(value) && br_number == 0); ¤ ¥\circled{4}\circlednote{4}{checks that the size of each layer is equal to the number of brain regions. The size of each layer is the length of time series by the number of regions.}¥
if check
    msg = 'All ok!';
else   
...
%%%% ¡check_value!
br_number = sub.get('BA').get('BR_DICT').get('LENGTH');
¤check = (iscolumn(value) && isequal(size(value), [br_number, 1])) || (isempty(value) && br_number == 0);¤ ¥\circled{1}\circlednote{1}{checks that the size of the row of \code{value} is equal to the number of brain regions. The number of columns is 1.}¥¤
if check
    msg = 'All ok!';
else   
...
for i = 1:1:50
    sub = SubjectST( ...
        ...
        'BA', ba, ...
        ¤'ST', rand(ba.get('BR_DICT').get('LENGTH'), 1) ...¤ ¥\circled{1}\circlednote{1}{constructs the random adjacency matrix with a size of number of brain regions by 1.}¥¤
        );
    sub.memorize('VOI_DICT').get('ADD', VOINumeric('ID', 'Age', 'V', 100 * rand()))
    sub.memorize('VOI_DICT').get('ADD', VOICategoric('ID', 'Sex', 'CATEGORIES', {'Female', 'Male'}, 'V', randi(2, 1)))
    gr.get('SUB_DICT').get('ADD', sub)
...
br_number = sub.get('BA').get('BR_DICT').get('LENGTH');
num_layers = sub.get('L');
¤check = (iscell(value) && isequal(length(value), num_layers)  && isequal( cellfun(@(v) size(v, 1), value), ones(1, num_layers) * br_number)) || (isempty(value) && br_number == 0); ¤ ¥\circled{4}\circlednote{4}{checks the size of each layer is equal to the number of brain regions. The size of each layer is the number of regions by 1.}¥
if check
    msg = 'All ok!';
else   
