...
%%%%% %%%%% %%%%% %%%%% %%%%%
\clearpage
\section{Importer and Exporter of the data (for SubjectCON)}

\subsection{importer of CON subject group from TXT (ImporterGroupSubjectCON\_TXT)}
...
%%% ¡description!
ImporterGroupSubjectCON_XLS imports a group of subjects with connectivity data from a series of XLS/XLSX files contained in a folder named "GROUP_ID". 
...
The variables of interest are from another XLS/XLSX file named "GROUP_ID.vois.xlsx" (if exisitng) consisting of the following columns: Subject ID (column 1), covariates (subsequent columns). 
The 1st row contains the headers, the 2nd row a string with the categorical variables of interest, and each subsequent row the values for each subject.
...
\subsection{importer of CON subject group from XLS/XLSX (ImporterGroupSubjectCON\_XLS)}

We will start by implementing in detail \code{ImporterGroupSubjectCON\_XLS}. The data should be stored in the folder 'Group1' and 'Group2', and the file format is '.xls' or '.xlsx'.
...
\clearpage
\begin{lstlisting}[	label=cd:m:ImporterGroupSubjectCON\_XLS:tests,
	caption={
...
\subsection{exporter of CON subject group from TXT (ExporterGroupSubjectCON\_TXT)}

We will start by implementing in detail \code{ExporterGroupSubjectCON\_TXT}. The data should be stored in the folder 'Group1' and 'Group2', and the file format is '.txt'.
...
%%% ¡description!
ExporterGroupSubjectCON_TXT exports a group of subjects with connectivity data to a series of tab-separated TXT files contained in a folder named "GROUP_ID". 
All these files are saved in the same folder. Each file contains a table of values corresponding to the adjacency matrix. 
The variables of interest (if existing) are saved in another tab-separated TXT file named "GROUP_ID.vois.txt" consisting of the following columns: Subject ID (column 1), covariates (subsequent columns). 
The 1st row contains the headers, the 2nd row a string with the categorical variables of interest, and each subsequent row the values for each subject.
...
%%%%% %%%%% %%%%% %%%%% %%%%%
\clearpage
\subsection{exporter of CON subject group from XLS/XLSX (ExporterGroupSubjectCON\_XLS)}

We will start by implementing in detail \code{ExporterGroupSubjectCON\_XLS}. The data should be stored in the folder 'Group1' and 'Group2', and the file format is '.txt'.
...
We will start by implementing in detail \code{SubjectCON\_FUN\_MP}. The connectivity matrix can be obtained from DTI data and the functional matrix can be obtained from fMRI data.
