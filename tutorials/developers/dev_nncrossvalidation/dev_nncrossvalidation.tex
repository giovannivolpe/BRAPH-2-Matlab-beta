...
In this Tutorial, we will explain how to create the generator file \fn{*.gen.m} for a new neural network cross validation, which can then be compiled by \code{braph2genesis}. All kinds of neural network cross validation are (direct or indirect) extensions of the base element \code{NNCrossValidation}. Here, we will use as examples the cross validation for multi-layer perceptron regressors \code{NNRegressorMLP\_CrossValidation} and  the cross validation for multi-layer perceptron classifiers \code{NNClassifierMLP\_CrossValidation}.
\end{abstract}
