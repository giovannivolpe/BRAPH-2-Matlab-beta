...
%%% ¡prop! ¥\circled{2}\circlednote{2}{define the number of neuron per layer. For example, \code{[32 32]} repreents two layers, each contains 32 neurons.}¥
...
start = tic;
for i = 1:1:P
    rng(seeds(i), 'twister') ¥\circled{4}\circlednote{4}{iteratively shuffles the feature values from any given dataset P times and measuring the resulting average decrease in model performance.}¥
