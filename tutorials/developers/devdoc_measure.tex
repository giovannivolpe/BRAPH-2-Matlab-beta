...
%%%%% %%%%% %%%%% %%%%% %%%%%
\clearpage
\section{Implementation of Triangles}
Now we implement the nodal measure \code{Triangles}, again highlighting the differences.
The parts of the code that are modified are highlighted.
...
¤%% ¡header!¤
Triangles ¤< Measure (m, ¤triangls¤) ¤is the triangles.

¤%%% ¡description!¤
The triangles are calculated as the number of neighbours of a node that are also neighbours of each other within a layer. 
...
%%%% ¡default!
¤'The triangles are calculated as the number of neighbours of a node that are also neighbours of each other within a layer. In weighted graphs, the triangles are calculated as geometric mean of the weights of the edges forming the triangle.'¤
...
directionality\_type =   g.get('DIRECTIONALITY\_TYPE', L);
for li = 1:1:L        
Aii = A{li, li};    
...
directed_triangles_rule = m.get('RULE'); ¥\circled{1}\circlednote{1}{check for measure Triangle rules in \circled{2}.}¥
...
¤%% ¡props!
%%% ¡prop! 
rule (parameter, OPTION) is the rule to determine what is a triangle in directed networks. ¥\circled{2}\circlednote{2}{these are the available triangle rules.}¥
%%%% ¡settings!
