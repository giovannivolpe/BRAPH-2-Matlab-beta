...
To open the GUI and upload the brain connectivity data, you can also do it from the command line (i.e., without opening an analysis pipeline) by typing the commands in \Coderef{cd:launch}. Moreover, if you don't have the \fn{Example data CON XLS} folder inside connectivity, then you can generate it by running the commands in \Coderef{cd:exampledata}.
%
\begin{lstlisting}[
	label=cd:launch,
	caption={
...
\begin{lstlisting}[
	...
	caption={
		{\bf Code to generate the example data folder in the connectivity directory.}
		This code can be used in the MatLab command line to copy the \fn{Example data CON XLS} folder to the connectivity pipeline folder.
	}
]
test_ImporterGroupSubjectCON_XLS ¥\circled{1}\circlednote{1}{copies the example connectivity XLS data folder.}¥
test_ImporterGroupSubjectCON_TXT ¥\circled{2}\circlednote{2}{copies the example connectivity TXT data folder.}¥
\end{lstlisting}
