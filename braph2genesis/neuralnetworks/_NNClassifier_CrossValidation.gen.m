...
if isempty(d_list)
    ...
    if ~isa(nncv.get('NN_TEMPLATE'), 'NoValue')
...
%%% ¡prop!
AVG_FEATURE_IMPORTANCE (result, cell) averages the feature importances across k folds.
%%%% ¡calculate!
e_list = nncv.get('EVALUATOR_LIST');

fi = cellfun(@(e) e.get('FEATURE_IMPORTANCE'), ...
    e_list, 'UniformOutput', false);

combined_fi = cellfun(@(x) cell2mat(x), c_matrices, 'UniformOutput', false);
value = mean(combined_fi);
