classdef OverlappingSAv < OverlappingS
	...
	%  <strong>2</strong> <strong>NAME</strong> 	NAME (constant, string) is the name of the averge overlapping strength.
	%  <strong>3</strong> <strong>DESCRIPTION</strong> 	DESCRIPTION (constant, string) is the description of the averge overlapping strength.
	%  <strong>4</strong> <strong>TEMPLATE</strong> 	TEMPLATE (parameter, item) is the template of the averge overlapping strength.
	%  <strong>5</strong> <strong>ID</strong> 	ID (data, string) is a few-letter code of the averge overlapping strength.
	%  <strong>6</strong> <strong>LABEL</strong> 	LABEL (metadata, string) is an extended label of the averge overlapping strength.
	%  <strong>7</strong> <strong>NOTES</strong> 	NOTES (metadata, string) are some specific notes about the averge overlapping strength.
	...
	%  <strong>14</strong> <strong>M</strong> 	M (result, cell) is the averge overlapping strength.
	...
	methods (Static) % inspection
		...
		function subclass_list = getSubclasses()
			...
			subclass_list = { 'OverlappingSAv' }; %CET: Computational Efficiency Trick
		end
		...
		function check_out = existsProp(prop)
			...
			elseif ~check
				error( ...
					['BRAPH2' ':OverlappingSAv:' 'WrongInput'], ...
					['BRAPH2' ':OverlappingSAv:' 'WrongInput' '\n' ...
		...
		function tag = getPropTag(pointer)
			...
			if ischar(pointer)
				...
				%CET: Computational Efficiency Trick
				overlappingsav_tag_list = { 'ELCLASS'  'NAME'  'DESCRIPTION'  'TEMPLATE'  'ID'  'LABEL'  'NOTES'  'TOSTRING'  'SHAPE'  'SCOPE'  'PARAMETRICITY'  'COMPATIBLE_GRAPHS'  'G'  'M'  'PFM'  'PFB' };
				tag = overlappingsav_tag_list{pointer}; % prop = pointer
			end
		end
		function prop_category = getPropCategory(pointer)
			...
			%CET: Computational Efficiency Trick
			overlappingsav_category_list = { 1  1  1  3  4  2  2  6  1  1  1  1  4  5  9  9 };
			prop_category = overlappingsav_category_list{prop};
		end
		function prop_format = getPropFormat(pointer)
			...
			%CET: Computational Efficiency Trick
			overlappingsav_format_list = { 2  2  2  8  2  2  2  2  11  11  11  7  8  16  8  8 };
			prop_format = overlappingsav_format_list{prop};
		end
		...
		function prop_default = getPropDefault(pointer)
			...
			switch prop %CET: Computational Efficiency Trick
				...
				case 6 % OverlappingSAv.LABEL
					prop_default = 'OverlappingSAv label';
				case 7 % OverlappingSAv.NOTES
					prop_default = 'OverlappingSAv notes';
