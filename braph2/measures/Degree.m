	...
	methods (Static) % inspection
		...
		function tag = getPropTag(pointer)
			...
			if ischar(pointer)
				...
				%CET: Computational Efficiency Trick
				degree_tag_list = { 'ELCLASS'  'NAME'  'DESCRIPTION'  'TEMPLATE'  'ID'  'LABEL'  'NOTES'  'TOSTRING'  'SHAPE'  'SCOPE'  'PARAMETRICITY'  'COMPATIBLE_GRAPHS'  'G'  'M'  'PFM'  'PFB' };
		...
		function prop_category = getPropCategory(pointer)
			...
			%CET: Computational Efficiency Trick
			degree_category_list = { 1  1  1  3  4  2  2  6  1  1  1  1  4  5  9  9 };
		...
		function prop_format = getPropFormat(pointer)
			...
			%CET: Computational Efficiency Trick
			degree_format_list = { 2  2  2  8  2  2  2  2  11  11  11  7  8  16  8  8 };
		...
		function prop_description = getPropDescription(pointer)
			...
			%CET: Computational Efficiency Trick
			degree_description_list = { 'ELCLASS (constant, string) is the class of the % % % .'  'NAME (constant, string) is the name of the degree.'  'DESCRIPTION (constant, string) is the description of the degree.'  'TEMPLATE (parameter, item) is the template of the degree.'  'ID (data, string) is a few-letter code of the degree.'  'LABEL (metadata, string) is an extended label of the degree.'  'NOTES (metadata, string) are some specific notes about the degree.'  'TOSTRING (query, string) returns a string that represents the object.'  'SHAPE (constant, scalar) is the measure shape Measure.NODAL.'  'SCOPE (constant, scalar) is the measure scope Measure.UNILAYER.'  'PARAMETRICITY (constant, scalar) is the parametricity of the measure Measure.NONPARAMETRIC.'  'COMPATIBLE_GRAPHS (constant, classlist) is the list of compatible graphs.'  'G (data, item) is the measure graph.'  'M (result, cell) is the degree.'  'PFM (gui, item) contains the panel figure of the measure.'  'PFB (gui, item) contains the panel figure of the brain measure.' };
