classdef WeightedMxPAv < WeightedMxP
	...
	methods (Static) % inspection
		...
		function subclass_list = getSubclasses()
			...
			subclass_list = subclasses('WeightedMxPAv', [], [], true);
		end
		function prop_list = getProps(category)
			...
			switch category
				case Category.CONSTANT
					prop_list = [ ...
						WeightedMxP.getProps(Category.CONSTANT) ...
						];
				case Category.METADATA
					prop_list = [ ...
						WeightedMxP.getProps(Category.METADATA) ...
						];
				case Category.PARAMETER
					prop_list = [ ...
						WeightedMxP.getProps(Category.PARAMETER) ...
						];
				case Category.DATA
					prop_list = [ ...
						WeightedMxP.getProps(Category.DATA) ...
						];
				case Category.RESULT
					prop_list = [
						WeightedMxP.getProps(Category.RESULT) ...
						];
				case Category.QUERY
					prop_list = [ ...
						WeightedMxP.getProps(Category.QUERY) ...
						];
				case Category.EVANESCENT
					prop_list = [ ...
						WeightedMxP.getProps(Category.EVANESCENT) ...
						];
				case Category.FIGURE
					prop_list = [ ...
						WeightedMxP.getProps(Category.FIGURE) ...
						];
				case Category.GUI
					prop_list = [ ...
						WeightedMxP.getProps(Category.GUI) ...
						];
			end
		end
		function prop_number = getPropNumber(varargin)
			...
			prop_number = numel(WeightedMxPAv.getProps(varargin{:}));
		end
		function check_out = existsProp(prop)
			...
			check = any(prop == WeightedMxPAv.getProps());
			
			if nargout == 1
				check_out = check;
			elseif ~check
				error( ...
					[BRAPH2.STR ':WeightedMxPAv:' BRAPH2.WRONG_INPUT], ...
					[BRAPH2.STR ':WeightedMxPAv:' BRAPH2.WRONG_INPUT '\n' ...
		...
		function check_out = existsTag(tag)
			...
			weightedmxpav_tag_list = cellfun(@(x) WeightedMxPAv.getPropTag(x), num2cell(WeightedMxPAv.getProps()), 'UniformOutput', false);
			check = any(strcmp(tag, weightedmxpav_tag_list));
		...
		function prop = getPropProp(pointer)
			...
			if ischar(pointer)
				weightedmxpav_tag_list = cellfun(@(x) WeightedMxPAv.getPropTag(x), num2cell(WeightedMxPAv.getProps()), 'UniformOutput', false);
				prop = find(strcmp(pointer, weightedmxpav_tag_list)); % tag = pointer
			else % numeric
				prop = pointer;
			end
		end
		function tag = getPropTag(pointer)
			...
			if ischar(pointer)
				...
				switch prop
					otherwise
						tag = getPropTag@WeightedMxP(prop);
				end
			end
		end
		function prop_category = getPropCategory(pointer)
			...
			switch prop
				otherwise
					prop_category = getPropCategory@WeightedMxP(prop);
			end
		end
		function prop_format = getPropFormat(pointer)
			...
			switch prop
				otherwise
					prop_format = getPropFormat@WeightedMxP(prop);
			end
		end
		...
		function prop_default = getPropDefault(pointer)
			...
			switch prop
				...
				case WeightedMxPAv.TEMPLATE
					prop_default = Format.getFormatDefault(Format.ITEM, WeightedMxPAv.getPropSettings(prop));
				case WeightedMxPAv.ID
	...
	methods (Static) % checkProp
		function prop_check = checkProp(pointer, value)
			...
			%  Error id: €BRAPH2.STR€:WeightedMxPAv:€BRAPH2.WRONG_INPUT€
			% 
			% Alternative forms to call this method are (POINTER = PROP or TAG):
			...
			switch prop
				case WeightedMxPAv.TEMPLATE % __WeightedMxPAv.TEMPLATE__
					check = Format.checkFormat(Format.ITEM, value, WeightedMxPAv.getPropSettings(prop));
				otherwise
					if prop <= WeightedMxP.getPropNumber()
	...
	methods (Access=protected) % calculate value
		function value = calculateValue(m, prop, varargin)
			...
			switch prop
				case WeightedMxPAv.M % __WeightedMxPAv.M__
					rng_settings_ = rng(); rng(m.getPropSeed(WeightedMxPAv.M), 'twister')
