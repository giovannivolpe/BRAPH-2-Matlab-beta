classdef DegreeOverlapAv < DegreeOverlap
	...
	methods (Static) % inspection
		...
		function subclass_list = getSubclasses()
			...
			subclass_list = subclasses('DegreeOverlapAv', [], [], true);
		end
		function prop_list = getProps(category)
			...
			switch category
				case Category.CONSTANT
					prop_list = [ ...
						DegreeOverlap.getProps(Category.CONSTANT) ...
						];
				case Category.METADATA
					prop_list = [ ...
						DegreeOverlap.getProps(Category.METADATA) ...
						];
				case Category.PARAMETER
					prop_list = [ ...
						DegreeOverlap.getProps(Category.PARAMETER) ...
						];
				case Category.DATA
					prop_list = [ ...
						DegreeOverlap.getProps(Category.DATA) ...
						];
				case Category.RESULT
					prop_list = [
						DegreeOverlap.getProps(Category.RESULT) ...
						];
				case Category.QUERY
					prop_list = [ ...
						DegreeOverlap.getProps(Category.QUERY) ...
						];
				case Category.EVANESCENT
					prop_list = [ ...
						DegreeOverlap.getProps(Category.EVANESCENT) ...
						];
				case Category.FIGURE
					prop_list = [ ...
						DegreeOverlap.getProps(Category.FIGURE) ...
						];
				case Category.GUI
					prop_list = [ ...
						DegreeOverlap.getProps(Category.GUI) ...
						];
			end
		end
		function prop_number = getPropNumber(varargin)
			...
			prop_number = numel(DegreeOverlapAv.getProps(varargin{:}));
		end
		function check_out = existsProp(prop)
			...
			check = any(prop == DegreeOverlapAv.getProps());
			
			if nargout == 1
				check_out = check;
			elseif ~check
				error( ...
					[BRAPH2.STR ':DegreeOverlapAv:' BRAPH2.WRONG_INPUT], ...
					[BRAPH2.STR ':DegreeOverlapAv:' BRAPH2.WRONG_INPUT '\n' ...
		...
		function check_out = existsTag(tag)
			...
			degreeoverlapav_tag_list = cellfun(@(x) DegreeOverlapAv.getPropTag(x), num2cell(DegreeOverlapAv.getProps()), 'UniformOutput', false);
			check = any(strcmp(tag, degreeoverlapav_tag_list));
		...
		function prop = getPropProp(pointer)
			...
			if ischar(pointer)
				degreeoverlapav_tag_list = cellfun(@(x) DegreeOverlapAv.getPropTag(x), num2cell(DegreeOverlapAv.getProps()), 'UniformOutput', false);
				prop = find(strcmp(pointer, degreeoverlapav_tag_list)); % tag = pointer
			else % numeric
				prop = pointer;
			end
		end
		function tag = getPropTag(pointer)
			...
			if ischar(pointer)
				...
				switch prop
					otherwise
						tag = getPropTag@DegreeOverlap(prop);
				end
			end
		end
		function prop_category = getPropCategory(pointer)
			...
			switch prop
				otherwise
					prop_category = getPropCategory@DegreeOverlap(prop);
			end
		end
		function prop_format = getPropFormat(pointer)
			...
			switch prop
				otherwise
					prop_format = getPropFormat@DegreeOverlap(prop);
			end
		end
		function prop_description = getPropDescription(pointer)
			...
			switch prop
				case DegreeOverlapAv.NAME
					prop_description = 'NAME (constant, string) is the name of the average degree overlap.';
				case DegreeOverlapAv.DESCRIPTION
					prop_description = 'DESCRIPTION (constant, string) is the description of the average degree overlap.';
				case DegreeOverlapAv.TEMPLATE
					prop_description = 'TEMPLATE (parameter, item) is the template of the average degree overlap.';
				case DegreeOverlapAv.ID
					prop_description = 'ID (data, string) is a few-letter code of the average degree overlap.';
				case DegreeOverlapAv.LABEL
					prop_description = 'LABEL (metadata, string) is an extended label of the average degree overlap.';
				case DegreeOverlapAv.NOTES
					prop_description = 'NOTES (metadata, string) are some specific notes about the average degree overlap.';
				case DegreeOverlapAv.SHAPE
					prop_description = 'SHAPE (constant, scalar) is the measure shape __Measure.NODAL__.';
				case DegreeOverlapAv.SCOPE
					prop_description = 'SCOPE (constant, scalar) is the measure scope __Measure.UNILAYER__.';
				case DegreeOverlapAv.PARAMETRICITY
					prop_description = 'PARAMETRICITY (constant, scalar) is the parametricity of the measure __Measure.NONPARAMETRIC__.';
				case DegreeOverlapAv.COMPATIBLE_GRAPHS
					prop_description = 'COMPATIBLE_GRAPHS (constant, classlist) is the list of compatible graphs.';
				case DegreeOverlapAv.M
					prop_description = 'M (result, cell) is the average degree overlap.';
				otherwise
					prop_description = getPropDescription@DegreeOverlap(prop);
			end
		end
		...
		function prop_default = getPropDefault(pointer)
			...
			switch prop
				...
				case DegreeOverlapAv.TEMPLATE
					prop_default = Format.getFormatDefault(Format.ITEM, DegreeOverlapAv.getPropSettings(prop));
				case DegreeOverlapAv.ID
	...
	methods (Static) % checkProp
		function prop_check = checkProp(pointer, value)
			...
			%  Error id: €BRAPH2.STR€:DegreeOverlapAv:€BRAPH2.WRONG_INPUT€
			% 
			% Alternative forms to call this method are (POINTER = PROP or TAG):
			...
			switch prop
				case DegreeOverlapAv.TEMPLATE % __DegreeOverlapAv.TEMPLATE__
					check = Format.checkFormat(Format.ITEM, value, DegreeOverlapAv.getPropSettings(prop));
				otherwise
					if prop <= DegreeOverlap.getPropNumber()
	...
	methods (Access=protected) % calculate value
		function value = calculateValue(m, prop, varargin)
			...
			switch prop
				case DegreeOverlapAv.M % __DegreeOverlapAv.M__
					rng_settings_ = rng(); rng(m.getPropSeed(DegreeOverlapAv.M), 'twister')
