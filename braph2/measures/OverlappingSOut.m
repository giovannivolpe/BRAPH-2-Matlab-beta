classdef OverlappingSOut < StrengthOut
	...
	methods (Static) % inspection
		...
		function subclass_list = getSubclasses()
			...
			subclass_list = subclasses('OverlappingSOut', [], [], true);
		end
		...
		function prop_number = getPropNumber(varargin)
			...
			prop_number = numel(OverlappingSOut.getProps(varargin{:}));
		end
		function check_out = existsProp(prop)
			...
			check = any(prop == OverlappingSOut.getProps());
			
			if nargout == 1
				check_out = check;
			elseif ~check
				error( ...
					[BRAPH2.STR ':OverlappingSOut:' BRAPH2.WRONG_INPUT], ...
					[BRAPH2.STR ':OverlappingSOut:' BRAPH2.WRONG_INPUT '\n' ...
		...
		function check_out = existsTag(tag)
			...
			overlappingsout_tag_list = cellfun(@(x) OverlappingSOut.getPropTag(x), num2cell(OverlappingSOut.getProps()), 'UniformOutput', false);
			check = any(strcmp(tag, overlappingsout_tag_list));
		...
		function prop = getPropProp(pointer)
			...
			if ischar(pointer)
				overlappingsout_tag_list = cellfun(@(x) OverlappingSOut.getPropTag(x), num2cell(OverlappingSOut.getProps()), 'UniformOutput', false);
				prop = find(strcmp(pointer, overlappingsout_tag_list)); % tag = pointer
			else % numeric
				prop = pointer;
			end
		end
		...
		function prop_default = getPropDefault(pointer)
			...
			switch prop
				...
				case OverlappingSOut.TEMPLATE
					prop_default = Format.getFormatDefault(Format.ITEM, OverlappingSOut.getPropSettings(prop));
				case OverlappingSOut.ID
	...
	methods (Static) % checkProp
		function prop_check = checkProp(pointer, value)
			...
			%  Error id: €BRAPH2.STR€:OverlappingSOut:€BRAPH2.WRONG_INPUT€
			% 
			% Alternative forms to call this method are (POINTER = PROP or TAG):
			...
			switch prop
				case OverlappingSOut.TEMPLATE % __OverlappingSOut.TEMPLATE__
					check = Format.checkFormat(Format.ITEM, value, OverlappingSOut.getPropSettings(prop));
				otherwise
	...
	methods (Access=protected) % calculate value
		function value = calculateValue(m, prop, varargin)
			...
			switch prop
				case OverlappingSOut.M % __OverlappingSOut.M__
					rng_settings_ = rng(); rng(m.getPropSeed(OverlappingSOut.M), 'twister')
