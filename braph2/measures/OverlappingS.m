classdef OverlappingS < Strength
	...
	methods (Static) % inspection
		...
		function subclass_list = getSubclasses()
			...
			subclass_list = subclasses('OverlappingS', [], [], true);
		end
		...
		function prop_number = getPropNumber(varargin)
			...
			prop_number = numel(OverlappingS.getProps(varargin{:}));
		end
		function check_out = existsProp(prop)
			...
			check = any(prop == OverlappingS.getProps());
			
			if nargout == 1
				check_out = check;
			elseif ~check
				error( ...
					[BRAPH2.STR ':OverlappingS:' BRAPH2.WRONG_INPUT], ...
					[BRAPH2.STR ':OverlappingS:' BRAPH2.WRONG_INPUT '\n' ...
		...
		function check_out = existsTag(tag)
			...
			overlappings_tag_list = cellfun(@(x) OverlappingS.getPropTag(x), num2cell(OverlappingS.getProps()), 'UniformOutput', false);
			check = any(strcmp(tag, overlappings_tag_list));
		...
		function prop = getPropProp(pointer)
			...
			if ischar(pointer)
				overlappings_tag_list = cellfun(@(x) OverlappingS.getPropTag(x), num2cell(OverlappingS.getProps()), 'UniformOutput', false);
				prop = find(strcmp(pointer, overlappings_tag_list)); % tag = pointer
			else % numeric
				prop = pointer;
			end
		end
		...
		function prop_default = getPropDefault(pointer)
			...
			switch prop
				...
				case OverlappingS.TEMPLATE
					prop_default = Format.getFormatDefault(Format.ITEM, OverlappingS.getPropSettings(prop));
				case OverlappingS.ID
	...
	methods (Static) % checkProp
		function prop_check = checkProp(pointer, value)
			...
			%  Error id: €BRAPH2.STR€:OverlappingS:€BRAPH2.WRONG_INPUT€
			% 
			% Alternative forms to call this method are (POINTER = PROP or TAG):
			...
			switch prop
				case OverlappingS.TEMPLATE % __OverlappingS.TEMPLATE__
					check = Format.checkFormat(Format.ITEM, value, OverlappingS.getPropSettings(prop));
				otherwise
	...
	methods (Access=protected) % calculate value
		function value = calculateValue(m, prop, varargin)
			...
			switch prop
				case OverlappingS.M % __OverlappingS.M__
					rng_settings_ = rng(); rng(m.getPropSeed(OverlappingS.M), 'twister')
