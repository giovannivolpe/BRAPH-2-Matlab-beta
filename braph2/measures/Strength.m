	...
	methods (Static) % inspection
		...
		function tag = getPropTag(pointer)
			...
			if ischar(pointer)
				...
				%CET: Computational Efficiency Trick
				strength_tag_list = { 'ELCLASS'  'NAME'  'DESCRIPTION'  'TEMPLATE'  'ID'  'LABEL'  'NOTES'  'TOSTRING'  'SHAPE'  'SCOPE'  'PARAMETRICITY'  'COMPATIBLE_GRAPHS'  'G'  'M'  'PFM'  'PFB' };
		...
		function prop_category = getPropCategory(pointer)
			...
			%CET: Computational Efficiency Trick
			strength_category_list = { 1  1  1  3  4  2  2  6  1  1  1  1  4  5  9  9 };
		...
		function prop_format = getPropFormat(pointer)
			...
			%CET: Computational Efficiency Trick
			strength_format_list = { 2  2  2  8  2  2  2  2  11  11  11  7  8  16  8  8 };
		...
		function prop_description = getPropDescription(pointer)
			...
			%CET: Computational Efficiency Trick
			strength_description_list = { 'ELCLASS (constant, string) is the class of the % % % .'  'NAME (constant, string) is the name of the strength.'  'DESCRIPTION (constant, string) is the description of the strength.'  'TEMPLATE (parameter, item) is the template of the strength.'  'ID (data, string) is a few-letter code of the strength.'  'LABEL (metadata, string) is an extended label of the strength.'  'NOTES (metadata, string) are some specific notes about the strength.'  'TOSTRING (query, string) returns a string that represents the object.'  'SHAPE (constant, scalar) is the measure shape Measure.NODAL.'  'SCOPE (constant, scalar) is the measure scope Measure.UNILAYER.'  'PARAMETRICITY (constant, scalar) is the parametricity of the measure Measure.NONPARAMETRIC.'  'COMPATIBLE_GRAPHS (constant, classlist) is the list of compatible graphs.'  'G (data, item) is the measure graph.'  'M (result, cell) is the strength.'  'PFM (gui, item) contains the panel figure of the measure.'  'PFB (gui, item) contains the panel figure of the brain measure.' };
