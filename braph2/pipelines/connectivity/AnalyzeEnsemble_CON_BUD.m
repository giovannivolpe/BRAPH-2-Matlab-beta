classdef AnalyzeEnsemble_CON_BUD < AnalyzeEnsemble
	...
	methods (Static) % inspection
		...
		function subclass_list = getSubclasses()
			...
			subclass_list = subclasses('AnalyzeEnsemble_CON_BUD', [], [], true);
		end
		...
		function prop_number = getPropNumber(varargin)
			...
			prop_number = numel(AnalyzeEnsemble_CON_BUD.getProps(varargin{:}));
		end
		function check_out = existsProp(prop)
			...
			check = any(prop == AnalyzeEnsemble_CON_BUD.getProps());
			
			if nargout == 1
				check_out = check;
			elseif ~check
				error( ...
					[BRAPH2.STR ':AnalyzeEnsemble_CON_BUD:' BRAPH2.WRONG_INPUT], ...
					[BRAPH2.STR ':AnalyzeEnsemble_CON_BUD:' BRAPH2.WRONG_INPUT '\n' ...
		...
		function check_out = existsTag(tag)
			...
			analyzeensemble_con_bud_tag_list = cellfun(@(x) AnalyzeEnsemble_CON_BUD.getPropTag(x), num2cell(AnalyzeEnsemble_CON_BUD.getProps()), 'UniformOutput', false);
			check = any(strcmp(tag, analyzeensemble_con_bud_tag_list));
		...
		function prop = getPropProp(pointer)
			...
			if ischar(pointer)
				analyzeensemble_con_bud_tag_list = cellfun(@(x) AnalyzeEnsemble_CON_BUD.getPropTag(x), num2cell(AnalyzeEnsemble_CON_BUD.getProps()), 'UniformOutput', false);
				prop = find(strcmp(pointer, analyzeensemble_con_bud_tag_list)); % tag = pointer
			else % numeric
				prop = pointer;
			end
		end
		function tag = getPropTag(pointer)
			...
			if ischar(pointer)
				...
				switch prop
					case AnalyzeEnsemble_CON_BUD.DENSITIES
						tag = AnalyzeEnsemble_CON_BUD.DENSITIES_TAG;
					otherwise
		...
		function prop_category = getPropCategory(pointer)
			...
			switch prop
				case AnalyzeEnsemble_CON_BUD.DENSITIES
					prop_category = AnalyzeEnsemble_CON_BUD.DENSITIES_CATEGORY;
				otherwise
		...
		function prop_format = getPropFormat(pointer)
			...
			switch prop
				case AnalyzeEnsemble_CON_BUD.DENSITIES
					prop_format = AnalyzeEnsemble_CON_BUD.DENSITIES_FORMAT;
				otherwise
		...
		function prop_description = getPropDescription(pointer)
			...
			switch prop
				...
				case AnalyzeEnsemble_CON_BUD.NAME
					prop_description = 'NAME (constant, string) is the name of the ensenmble-based graph analysis using connectivity data of fixed density.';
				case AnalyzeEnsemble_CON_BUD.DESCRIPTION
					prop_description = 'DESCRIPTION (constant, string) is the description of the ensenmble-based graph analysis using connectivity data of fixed density.';
				case AnalyzeEnsemble_CON_BUD.TEMPLATE
					prop_description = 'TEMPLATE (parameter, item) is the template of the ensenmble-based graph analysis using connectivity data of fixed density.';
				case AnalyzeEnsemble_CON_BUD.ID
					prop_description = 'ID (data, string) is a few-letter code for the ensenmble-based graph analysis using connectivity data of fixed density.';
				case AnalyzeEnsemble_CON_BUD.LABEL
					prop_description = 'LABEL (metadata, string) is an extended label of the ensenmble-based graph analysis using connectivity data of fixed density.';
				case AnalyzeEnsemble_CON_BUD.NOTES
					prop_description = 'NOTES (metadata, string) are some specific notes about the ensenmble-based graph analysis using connectivity data of fixed density.';
				case AnalyzeEnsemble_CON_BUD.GR
					prop_description = 'GR (data, item) is the subject group, which also defines the subject class SubjectCON.';
				case AnalyzeEnsemble_CON_BUD.GRAPH_TEMPLATE
					prop_description = 'GRAPH_TEMPLATE (parameter, item) is the graph template to set all graph and measure parameters.';
				case AnalyzeEnsemble_CON_BUD.G_DICT
					prop_description = 'G_DICT (result, idict) is the graph (MultigraphBUD) ensemble obtained from this analysis.';
				case AnalyzeEnsemble_CON_BUD.ME_DICT
		...
		function prop_default = getPropDefault(pointer)
			...
			switch prop
				...
				case AnalyzeEnsemble_CON_BUD.TEMPLATE
					prop_default = Format.getFormatDefault(Format.ITEM, AnalyzeEnsemble_CON_BUD.getPropSettings(prop));
				case AnalyzeEnsemble_CON_BUD.ID
				...
				case AnalyzeEnsemble_CON_BUD.GRAPH_TEMPLATE
					prop_default = Format.getFormatDefault(Format.ITEM, AnalyzeEnsemble_CON_BUD.getPropSettings(prop));
				case AnalyzeEnsemble_CON_BUD.G_DICT
					prop_default = Format.getFormatDefault(Format.IDICT, AnalyzeEnsemble_CON_BUD.getPropSettings(prop));
				otherwise
	...
	methods (Static) % checkProp
		function prop_check = checkProp(pointer, value)
			...
			%  Error id: €BRAPH2.STR€:AnalyzeEnsemble_CON_BUD:€BRAPH2.WRONG_INPUT€
			% 
			% Alternative forms to call this method are (POINTER = PROP or TAG):
			...
			switch prop
				case AnalyzeEnsemble_CON_BUD.DENSITIES % __AnalyzeEnsemble_CON_BUD.DENSITIES__
					check = Format.checkFormat(Format.RVECTOR, value, AnalyzeEnsemble_CON_BUD.getPropSettings(prop));
				case AnalyzeEnsemble_CON_BUD.TEMPLATE % __AnalyzeEnsemble_CON_BUD.TEMPLATE__
					check = Format.checkFormat(Format.ITEM, value, AnalyzeEnsemble_CON_BUD.getPropSettings(prop));
				case AnalyzeEnsemble_CON_BUD.GRAPH_TEMPLATE % __AnalyzeEnsemble_CON_BUD.GRAPH_TEMPLATE__
					check = Format.checkFormat(Format.ITEM, value, AnalyzeEnsemble_CON_BUD.getPropSettings(prop));
				case AnalyzeEnsemble_CON_BUD.G_DICT % __AnalyzeEnsemble_CON_BUD.G_DICT__
					check = Format.checkFormat(Format.IDICT, value, AnalyzeEnsemble_CON_BUD.getPropSettings(prop));
				otherwise
	...
	methods (Access=protected) % calculate value
		function value = calculateValue(a, prop, varargin)
			...
			switch prop
				case AnalyzeEnsemble_CON_BUD.G_DICT % __AnalyzeEnsemble_CON_BUD.G_DICT__
					rng_settings_ = rng(); rng(a.getPropSeed(AnalyzeEnsemble_CON_BUD.G_DICT), 'twister')
