classdef AnalyzeEnsemble_CON_WU < AnalyzeEnsemble
	%AnalyzeEnsemble_CON_WU is a ensenmble-based graph analysis using connectivity data.
	...
	methods (Static) % inspection
		...
		function subclass_list = getSubclasses()
			...
			subclass_list = subclasses('AnalyzeEnsemble_CON_WU', [], [], true);
		end
		...
		function prop_number = getPropNumber(varargin)
			...
			prop_number = numel(AnalyzeEnsemble_CON_WU.getProps(varargin{:}));
		end
		function check_out = existsProp(prop)
			...
			check = any(prop == AnalyzeEnsemble_CON_WU.getProps());
		...
		function check_out = existsTag(tag)
			...
			analyzeensemble_con_wu_tag_list = cellfun(@(x) AnalyzeEnsemble_CON_WU.getPropTag(x), num2cell(AnalyzeEnsemble_CON_WU.getProps()), 'UniformOutput', false);
			check = any(strcmp(tag, analyzeensemble_con_wu_tag_list));
		...
		function prop = getPropProp(pointer)
			...
			if ischar(pointer)
				analyzeensemble_con_wu_tag_list = cellfun(@(x) AnalyzeEnsemble_CON_WU.getPropTag(x), num2cell(AnalyzeEnsemble_CON_WU.getProps()), 'UniformOutput', false);
				prop = find(strcmp(pointer, analyzeensemble_con_wu_tag_list)); % tag = pointer
			else % numeric
				prop = pointer;
			end
		end
		...
		function prop_description = getPropDescription(pointer)
			...
			switch prop
				...
				case AnalyzeEnsemble_CON_WU.NOTES
				...
				case AnalyzeEnsemble_CON_WU.GRAPH_TEMPLATE
		...
		function prop_default = getPropDefault(pointer)
			...
			switch prop
				...
				case AnalyzeEnsemble_CON_WU.TEMPLATE
					prop_default = Format.getFormatDefault(Format.ITEM, AnalyzeEnsemble_CON_WU.getPropSettings(prop));
				case AnalyzeEnsemble_CON_WU.ID
				...
				case AnalyzeEnsemble_CON_WU.GRAPH_TEMPLATE
					prop_default = Format.getFormatDefault(Format.ITEM, AnalyzeEnsemble_CON_WU.getPropSettings(prop));
				case AnalyzeEnsemble_CON_WU.G_DICT
					prop_default = Format.getFormatDefault(Format.IDICT, AnalyzeEnsemble_CON_WU.getPropSettings(prop));
				otherwise
	...
	methods (Static) % checkProp
		function prop_check = checkProp(pointer, value)
			...
			%  Error id: €BRAPH2.STR€:AnalyzeEnsemble_CON_WU:€BRAPH2.WRONG_INPUT€
			% 
			% Alternative forms to call this method are (POINTER = PROP or TAG):
			...
			switch prop
				case AnalyzeEnsemble_CON_WU.TEMPLATE % __AnalyzeEnsemble_CON_WU.TEMPLATE__
					check = Format.checkFormat(Format.ITEM, value, AnalyzeEnsemble_CON_WU.getPropSettings(prop));
				case AnalyzeEnsemble_CON_WU.GRAPH_TEMPLATE % __AnalyzeEnsemble_CON_WU.GRAPH_TEMPLATE__
					check = Format.checkFormat(Format.ITEM, value, AnalyzeEnsemble_CON_WU.getPropSettings(prop));
				case AnalyzeEnsemble_CON_WU.G_DICT % __AnalyzeEnsemble_CON_WU.G_DICT__
	...
	methods (Access=protected) % calculate value
		function value = calculateValue(a, prop, varargin)
			...
			switch prop
				case AnalyzeEnsemble_CON_WU.G_DICT % __AnalyzeEnsemble_CON_WU.G_DICT__
					rng_settings_ = rng(); rng(a.getPropSeed(AnalyzeEnsemble_CON_WU.G_DICT), 'twister')
