classdef AnalyzeGroup_ST_BUD < AnalyzeGroup
	...
	methods (Static) % inspection
		...
		function subclass_list = getSubclasses()
			...
			subclass_list = subclasses('AnalyzeGroup_ST_BUD', [], [], true);
		end
		...
		function prop_number = getPropNumber(varargin)
			...
			prop_number = numel(AnalyzeGroup_ST_BUD.getProps(varargin{:}));
		end
		function check_out = existsProp(prop)
			...
			check = any(prop == AnalyzeGroup_ST_BUD.getProps());
		...
		function check_out = existsTag(tag)
			...
			analyzegroup_st_bud_tag_list = cellfun(@(x) AnalyzeGroup_ST_BUD.getPropTag(x), num2cell(AnalyzeGroup_ST_BUD.getProps()), 'UniformOutput', false);
			check = any(strcmp(tag, analyzegroup_st_bud_tag_list));
		...
		function prop = getPropProp(pointer)
			...
			if ischar(pointer)
				analyzegroup_st_bud_tag_list = cellfun(@(x) AnalyzeGroup_ST_BUD.getPropTag(x), num2cell(AnalyzeGroup_ST_BUD.getProps()), 'UniformOutput', false);
				prop = find(strcmp(pointer, analyzegroup_st_bud_tag_list)); % tag = pointer
			else % numeric
				prop = pointer;
			end
		end
		...
		function prop_description = getPropDescription(pointer)
			...
			switch prop
				...
				case AnalyzeGroup_ST_BUD.NAME
					prop_description = 'NAME (constant, string) is the name of the group-based graph analysis with structural data at fixed density.';
				case AnalyzeGroup_ST_BUD.DESCRIPTION
					prop_description = 'DESCRIPTION (constant, string) is the description of the group-based graph analysis with structural data at fixed density.';
				case AnalyzeGroup_ST_BUD.TEMPLATE
					prop_description = 'TEMPLATE (parameter, item) is the template of the group-based graph analysis with structural data at fixed density.';
				case AnalyzeGroup_ST_BUD.ID
					prop_description = 'ID (data, string) is a few-letter code for the group-based graph analysis with structural data at fixed density.';
				case AnalyzeGroup_ST_BUD.LABEL
					prop_description = 'LABEL (metadata, string) is an extended label of the group-based graph analysis with structural data at fixed density.';
				case AnalyzeGroup_ST_BUD.NOTES
					prop_description = 'NOTES (metadata, string) are some specific notes about the group-based graph analysis with structural data at fixed density.';
		...
		function prop_default = getPropDefault(pointer)
			...
			switch prop
				...
				case AnalyzeGroup_ST_BUD.TEMPLATE
					prop_default = Format.getFormatDefault(Format.ITEM, AnalyzeGroup_ST_BUD.getPropSettings(prop));
				case AnalyzeGroup_ST_BUD.ID
	...
	methods (Static) % checkProp
		function prop_check = checkProp(pointer, value)
			...
			%  Error id: €BRAPH2.STR€:AnalyzeGroup_ST_BUD:€BRAPH2.WRONG_INPUT€
			% 
			% Alternative forms to call this method are (POINTER = PROP or TAG):
			...
			switch prop
				case AnalyzeGroup_ST_BUD.CORRELATION_RULE % __AnalyzeGroup_ST_BUD.CORRELATION_RULE__
					check = Format.checkFormat(Format.OPTION, value, AnalyzeGroup_ST_BUD.getPropSettings(prop));
				case AnalyzeGroup_ST_BUD.NEGATIVE_WEIGHT_RULE % __AnalyzeGroup_ST_BUD.NEGATIVE_WEIGHT_RULE__
					check = Format.checkFormat(Format.OPTION, value, AnalyzeGroup_ST_BUD.getPropSettings(prop));
				case AnalyzeGroup_ST_BUD.DENSITIES % __AnalyzeGroup_ST_BUD.DENSITIES__
					check = Format.checkFormat(Format.RVECTOR, value, AnalyzeGroup_ST_BUD.getPropSettings(prop));
				case AnalyzeGroup_ST_BUD.TEMPLATE % __AnalyzeGroup_ST_BUD.TEMPLATE__
					check = Format.checkFormat(Format.ITEM, value, AnalyzeGroup_ST_BUD.getPropSettings(prop));
				case AnalyzeGroup_ST_BUD.G % __AnalyzeGroup_ST_BUD.G__
					check = Format.checkFormat(Format.ITEM, value, AnalyzeGroup_ST_BUD.getPropSettings(prop));
				otherwise
	...
	methods (Access=protected) % calculate value
		function value = calculateValue(a, prop, varargin)
			...
			switch prop
				case AnalyzeGroup_ST_BUD.G % __AnalyzeGroup_ST_BUD.G__
					rng_settings_ = rng(); rng(a.getPropSeed(AnalyzeGroup_ST_BUD.G), 'twister')
