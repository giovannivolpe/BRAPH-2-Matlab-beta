classdef ExporterGroupSubjectCON_MP_XLS < Exporter
	...
	methods (Static) % inspection
		...
		function subclass_list = getSubclasses()
			...
			subclass_list = subclasses('ExporterGroupSubjectCON_MP_XLS', [], [], true);
		end
		function prop_list = getProps(category)
			...
			if nargin == 0
				prop_list = [ ...
					Exporter.getProps() ...
						ExporterGroupSubjectCON_MP_XLS.GR ...
						ExporterGroupSubjectCON_MP_XLS.DIRECTORY ...
						ExporterGroupSubjectCON_MP_XLS.PUT_DIR ...
						ExporterGroupSubjectCON_MP_XLS.SAVE ...
						];
				return
			end
		...
		function prop_number = getPropNumber(varargin)
			...
			prop_number = numel(ExporterGroupSubjectCON_MP_XLS.getProps(varargin{:}));
		end
		function check_out = existsProp(prop)
			...
			check = any(prop == ExporterGroupSubjectCON_MP_XLS.getProps());
			
			if nargout == 1
				check_out = check;
			elseif ~check
				error( ...
					[BRAPH2.STR ':ExporterGroupSubjectCON_MP_XLS:' BRAPH2.WRONG_INPUT], ...
					[BRAPH2.STR ':ExporterGroupSubjectCON_MP_XLS:' BRAPH2.WRONG_INPUT '\n' ...
		...
		function check_out = existsTag(tag)
			...
			exportergroupsubjectcon_mp_xls_tag_list = cellfun(@(x) ExporterGroupSubjectCON_MP_XLS.getPropTag(x), num2cell(ExporterGroupSubjectCON_MP_XLS.getProps()), 'UniformOutput', false);
			check = any(strcmp(tag, exportergroupsubjectcon_mp_xls_tag_list));
		...
		function prop = getPropProp(pointer)
			...
			if ischar(pointer)
				exportergroupsubjectcon_mp_xls_tag_list = cellfun(@(x) ExporterGroupSubjectCON_MP_XLS.getPropTag(x), num2cell(ExporterGroupSubjectCON_MP_XLS.getProps()), 'UniformOutput', false);
				prop = find(strcmp(pointer, exportergroupsubjectcon_mp_xls_tag_list)); % tag = pointer
			else % numeric
				prop = pointer;
			end
		end
		function tag = getPropTag(pointer)
			...
			if ischar(pointer)
				...
				switch prop
					case ExporterGroupSubjectCON_MP_XLS.GR
						tag = ExporterGroupSubjectCON_MP_XLS.GR_TAG;
					case ExporterGroupSubjectCON_MP_XLS.DIRECTORY
						tag = ExporterGroupSubjectCON_MP_XLS.DIRECTORY_TAG;
					case ExporterGroupSubjectCON_MP_XLS.PUT_DIR
						tag = ExporterGroupSubjectCON_MP_XLS.PUT_DIR_TAG;
					case ExporterGroupSubjectCON_MP_XLS.SAVE
						tag = ExporterGroupSubjectCON_MP_XLS.SAVE_TAG;
					otherwise
		...
		function prop_category = getPropCategory(pointer)
			...
			switch prop
				case ExporterGroupSubjectCON_MP_XLS.GR
					prop_category = ExporterGroupSubjectCON_MP_XLS.GR_CATEGORY;
				case ExporterGroupSubjectCON_MP_XLS.DIRECTORY
					prop_category = ExporterGroupSubjectCON_MP_XLS.DIRECTORY_CATEGORY;
				case ExporterGroupSubjectCON_MP_XLS.PUT_DIR
					prop_category = ExporterGroupSubjectCON_MP_XLS.PUT_DIR_CATEGORY;
				case ExporterGroupSubjectCON_MP_XLS.SAVE
					prop_category = ExporterGroupSubjectCON_MP_XLS.SAVE_CATEGORY;
				otherwise
		...
		function prop_format = getPropFormat(pointer)
			...
			switch prop
				case ExporterGroupSubjectCON_MP_XLS.GR
					prop_format = ExporterGroupSubjectCON_MP_XLS.GR_FORMAT;
				case ExporterGroupSubjectCON_MP_XLS.DIRECTORY
					prop_format = ExporterGroupSubjectCON_MP_XLS.DIRECTORY_FORMAT;
				case ExporterGroupSubjectCON_MP_XLS.PUT_DIR
					prop_format = ExporterGroupSubjectCON_MP_XLS.PUT_DIR_FORMAT;
				case ExporterGroupSubjectCON_MP_XLS.SAVE
					prop_format = ExporterGroupSubjectCON_MP_XLS.SAVE_FORMAT;
				otherwise
		...
		function prop_description = getPropDescription(pointer)
			...
			switch prop
				...
				case ExporterGroupSubjectCON_MP_XLS.SAVE
					prop_description = 'SAVE (result, empty) saves the group of subjects with connectivity multiplex data in XLSX files in the selected directory.';
				case ExporterGroupSubjectCON_MP_XLS.NAME
					prop_description = 'NAME (constant, string) is the name of the CON MP subject group exporter in XLSX.';
				case ExporterGroupSubjectCON_MP_XLS.DESCRIPTION
					prop_description = 'DESCRIPTION (constant, string) is the description of the CON MP subject group exporter in XLSX.';
				case ExporterGroupSubjectCON_MP_XLS.TEMPLATE
					prop_description = 'TEMPLATE (parameter, item) is the template of the CON MP subject group exporter in XLSX.';
				case ExporterGroupSubjectCON_MP_XLS.ID
					prop_description = 'ID (data, string) is a few-letter code for the CON MP subject group exporter in XLSX.';
				case ExporterGroupSubjectCON_MP_XLS.LABEL
					prop_description = 'LABEL (metadata, string) is an extended label of the CON MP subject group exporter in XLSX.';
				case ExporterGroupSubjectCON_MP_XLS.NOTES
					prop_description = 'NOTES (metadata, string) are some specific notes about the CON MP subject group exporter in XLSX.';
				otherwise
		...
		function prop_default = getPropDefault(pointer)
			...
			switch prop
				...
				case ExporterGroupSubjectCON_MP_XLS.PUT_DIR
					prop_default = Format.getFormatDefault(Format.ITEM, ExporterGroupSubjectCON_MP_XLS.getPropSettings(prop));
				case ExporterGroupSubjectCON_MP_XLS.SAVE
					prop_default = Format.getFormatDefault(Format.EMPTY, ExporterGroupSubjectCON_MP_XLS.getPropSettings(prop));
				case ExporterGroupSubjectCON_MP_XLS.NAME
	...
	methods (Static) % checkProp
		function prop_check = checkProp(pointer, value)
			...
			%  Error id: €BRAPH2.STR€:ExporterGroupSubjectCON_MP_XLS:€BRAPH2.WRONG_INPUT€
			% 
			% Alternative forms to call this method are (POINTER = PROP or TAG):
			...
			switch prop
				case ExporterGroupSubjectCON_MP_XLS.GR % __ExporterGroupSubjectCON_MP_XLS.GR__
					check = Format.checkFormat(Format.ITEM, value, ExporterGroupSubjectCON_MP_XLS.getPropSettings(prop));
					if check
						check = any(strcmp(value.get(Group.SUB_CLASS_TAG), subclasses('SubjectCON_MP', [], [], true)));
					end
				case ExporterGroupSubjectCON_MP_XLS.DIRECTORY % __ExporterGroupSubjectCON_MP_XLS.DIRECTORY__
					check = Format.checkFormat(Format.STRING, value, ExporterGroupSubjectCON_MP_XLS.getPropSettings(prop));
				case ExporterGroupSubjectCON_MP_XLS.PUT_DIR % __ExporterGroupSubjectCON_MP_XLS.PUT_DIR__
					check = Format.checkFormat(Format.ITEM, value, ExporterGroupSubjectCON_MP_XLS.getPropSettings(prop));
				case ExporterGroupSubjectCON_MP_XLS.SAVE % __ExporterGroupSubjectCON_MP_XLS.SAVE__
					check = Format.checkFormat(Format.EMPTY, value, ExporterGroupSubjectCON_MP_XLS.getPropSettings(prop));
				case ExporterGroupSubjectCON_MP_XLS.TEMPLATE % __ExporterGroupSubjectCON_MP_XLS.TEMPLATE__
					check = Format.checkFormat(Format.ITEM, value, ExporterGroupSubjectCON_MP_XLS.getPropSettings(prop));
				otherwise
	...
	methods (Access=protected) % calculate value
		function value = calculateValue(ex, prop, varargin)
			...
			switch prop
				...
				case ExporterGroupSubjectCON_MP_XLS.SAVE % __ExporterGroupSubjectCON_MP_XLS.SAVE__
					rng_settings_ = rng(); rng(ex.getPropSeed(ExporterGroupSubjectCON_MP_XLS.SAVE), 'twister')
