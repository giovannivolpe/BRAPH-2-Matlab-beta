classdef ImporterGroupSubjectCON_MP_TXT < Importer
	...
	methods (Static) % inspection
		...
		function subclass_list = getSubclasses()
			...
			subclass_list = subclasses('ImporterGroupSubjectCON_MP_TXT', [], [], true);
		end
		function prop_list = getProps(category)
			...
			if nargin == 0
				prop_list = [ ...
					Importer.getProps() ...
						ImporterGroupSubjectCON_MP_TXT.DIRECTORY ...
						ImporterGroupSubjectCON_MP_TXT.GET_DIR ...
						ImporterGroupSubjectCON_MP_TXT.BA ...
						ImporterGroupSubjectCON_MP_TXT.GR ...
						];
				return
			end
		...
		function prop_number = getPropNumber(varargin)
			...
			prop_number = numel(ImporterGroupSubjectCON_MP_TXT.getProps(varargin{:}));
		end
		function check_out = existsProp(prop)
			...
			check = any(prop == ImporterGroupSubjectCON_MP_TXT.getProps());
			
			if nargout == 1
				check_out = check;
			elseif ~check
				error( ...
					[BRAPH2.STR ':ImporterGroupSubjectCON_MP_TXT:' BRAPH2.WRONG_INPUT], ...
					[BRAPH2.STR ':ImporterGroupSubjectCON_MP_TXT:' BRAPH2.WRONG_INPUT '\n' ...
		...
		function check_out = existsTag(tag)
			...
			importergroupsubjectcon_mp_txt_tag_list = cellfun(@(x) ImporterGroupSubjectCON_MP_TXT.getPropTag(x), num2cell(ImporterGroupSubjectCON_MP_TXT.getProps()), 'UniformOutput', false);
			check = any(strcmp(tag, importergroupsubjectcon_mp_txt_tag_list));
		...
		function prop = getPropProp(pointer)
			...
			if ischar(pointer)
				importergroupsubjectcon_mp_txt_tag_list = cellfun(@(x) ImporterGroupSubjectCON_MP_TXT.getPropTag(x), num2cell(ImporterGroupSubjectCON_MP_TXT.getProps()), 'UniformOutput', false);
				prop = find(strcmp(pointer, importergroupsubjectcon_mp_txt_tag_list)); % tag = pointer
			else % numeric
				prop = pointer;
			end
		end
		function tag = getPropTag(pointer)
			...
			if ischar(pointer)
				...
				switch prop
					case ImporterGroupSubjectCON_MP_TXT.DIRECTORY
						tag = ImporterGroupSubjectCON_MP_TXT.DIRECTORY_TAG;
					case ImporterGroupSubjectCON_MP_TXT.GET_DIR
						tag = ImporterGroupSubjectCON_MP_TXT.GET_DIR_TAG;
					case ImporterGroupSubjectCON_MP_TXT.BA
						tag = ImporterGroupSubjectCON_MP_TXT.BA_TAG;
					case ImporterGroupSubjectCON_MP_TXT.GR
						tag = ImporterGroupSubjectCON_MP_TXT.GR_TAG;
					otherwise
		...
		function prop_category = getPropCategory(pointer)
			...
			switch prop
				case ImporterGroupSubjectCON_MP_TXT.DIRECTORY
					prop_category = ImporterGroupSubjectCON_MP_TXT.DIRECTORY_CATEGORY;
				case ImporterGroupSubjectCON_MP_TXT.GET_DIR
					prop_category = ImporterGroupSubjectCON_MP_TXT.GET_DIR_CATEGORY;
				case ImporterGroupSubjectCON_MP_TXT.BA
					prop_category = ImporterGroupSubjectCON_MP_TXT.BA_CATEGORY;
				case ImporterGroupSubjectCON_MP_TXT.GR
					prop_category = ImporterGroupSubjectCON_MP_TXT.GR_CATEGORY;
				otherwise
		...
		function prop_format = getPropFormat(pointer)
			...
			switch prop
				case ImporterGroupSubjectCON_MP_TXT.DIRECTORY
					prop_format = ImporterGroupSubjectCON_MP_TXT.DIRECTORY_FORMAT;
				case ImporterGroupSubjectCON_MP_TXT.GET_DIR
					prop_format = ImporterGroupSubjectCON_MP_TXT.GET_DIR_FORMAT;
				case ImporterGroupSubjectCON_MP_TXT.BA
					prop_format = ImporterGroupSubjectCON_MP_TXT.BA_FORMAT;
				case ImporterGroupSubjectCON_MP_TXT.GR
					prop_format = ImporterGroupSubjectCON_MP_TXT.GR_FORMAT;
				otherwise
		...
		function prop_description = getPropDescription(pointer)
			...
			switch prop
				case ImporterGroupSubjectCON_MP_TXT.DIRECTORY
					prop_description = 'DIRECTORY (data, string) is the directory containing the CON MP subject group files from which to load the L layers of the subject group.';
				case ImporterGroupSubjectCON_MP_TXT.GET_DIR
				...
				case ImporterGroupSubjectCON_MP_TXT.NAME
					prop_description = 'NAME (constant, string) is the name of the CON MP subject group importer from TXT.';
				case ImporterGroupSubjectCON_MP_TXT.DESCRIPTION
					prop_description = 'DESCRIPTION (constant, string) is the description of the CON MP subject group importer from TXT.';
				case ImporterGroupSubjectCON_MP_TXT.TEMPLATE
					prop_description = 'TEMPLATE (parameter, item) is the template of the CON MP subject group importer from TXT.';
				case ImporterGroupSubjectCON_MP_TXT.ID
					prop_description = 'ID (data, string) is a few-letter code for the CON MP subject group importer from TXT.';
				case ImporterGroupSubjectCON_MP_TXT.LABEL
					prop_description = 'LABEL (metadata, string) is an extended label of the CON MP subject group importer from TXT.';
				case ImporterGroupSubjectCON_MP_TXT.NOTES
					prop_description = 'NOTES (metadata, string) are some specific notes about the CON MP subject group importer from TXT.';
				otherwise
		...
		function prop_default = getPropDefault(pointer)
			...
			switch prop
				...
				case ImporterGroupSubjectCON_MP_TXT.GET_DIR
					prop_default = Format.getFormatDefault(Format.ITEM, ImporterGroupSubjectCON_MP_TXT.getPropSettings(prop));
				case ImporterGroupSubjectCON_MP_TXT.BA
					prop_default = Format.getFormatDefault(Format.ITEM, ImporterGroupSubjectCON_MP_TXT.getPropSettings(prop));
				case ImporterGroupSubjectCON_MP_TXT.GR
	...
	methods (Static) % checkProp
		function prop_check = checkProp(pointer, value)
			...
			%  Error id: €BRAPH2.STR€:ImporterGroupSubjectCON_MP_TXT:€BRAPH2.WRONG_INPUT€
			% 
			% Alternative forms to call this method are (POINTER = PROP or TAG):
			...
			switch prop
				case ImporterGroupSubjectCON_MP_TXT.DIRECTORY % __ImporterGroupSubjectCON_MP_TXT.DIRECTORY__
					check = Format.checkFormat(Format.STRING, value, ImporterGroupSubjectCON_MP_TXT.getPropSettings(prop));
				case ImporterGroupSubjectCON_MP_TXT.GET_DIR % __ImporterGroupSubjectCON_MP_TXT.GET_DIR__
					check = Format.checkFormat(Format.ITEM, value, ImporterGroupSubjectCON_MP_TXT.getPropSettings(prop));
				case ImporterGroupSubjectCON_MP_TXT.BA % __ImporterGroupSubjectCON_MP_TXT.BA__
					check = Format.checkFormat(Format.ITEM, value, ImporterGroupSubjectCON_MP_TXT.getPropSettings(prop));
				case ImporterGroupSubjectCON_MP_TXT.GR % __ImporterGroupSubjectCON_MP_TXT.GR__
					check = Format.checkFormat(Format.ITEM, value, ImporterGroupSubjectCON_MP_TXT.getPropSettings(prop));
				case ImporterGroupSubjectCON_MP_TXT.TEMPLATE % __ImporterGroupSubjectCON_MP_TXT.TEMPLATE__
					check = Format.checkFormat(Format.ITEM, value, ImporterGroupSubjectCON_MP_TXT.getPropSettings(prop));
				otherwise
	...
	methods (Access=protected) % calculate value
		function value = calculateValue(im, prop, varargin)
			...
			switch prop
				...
				case ImporterGroupSubjectCON_MP_TXT.GR % __ImporterGroupSubjectCON_MP_TXT.GR__
					rng_settings_ = rng(); rng(im.getPropSeed(ImporterGroupSubjectCON_MP_TXT.GR), 'twister')
