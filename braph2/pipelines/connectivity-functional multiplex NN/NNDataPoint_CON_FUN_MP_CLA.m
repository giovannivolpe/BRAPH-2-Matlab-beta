classdef NNDataPoint_CON_FUN_MP_CLA < NNDataPoint
	...
	methods (Static) % inspection
		...
		function subclass_list = getSubclasses()
			...
			subclass_list = subclasses('NNDataPoint_CON_FUN_MP_CLA', [], [], true);
		end
			...
			if nargin == 0
				prop_list = [ ...
					NNDataPoint.getProps() ...
						...
						NNDataPoint_CON_FUN_MP_CLA.TARGET_IDS ...
						];
				return
			end
		...
		function prop_number = getPropNumber(varargin)
			...
			prop_number = numel(NNDataPoint_CON_FUN_MP_CLA.getProps(varargin{:}));
		end
		function check_out = existsProp(prop)
			...
			check = any(prop == NNDataPoint_CON_FUN_MP_CLA.getProps());
			
			if nargout == 1
				check_out = check;
			elseif ~check
				error( ...
					[BRAPH2.STR ':NNDataPoint_CON_FUN_MP_CLA:' BRAPH2.WRONG_INPUT], ...
					[BRAPH2.STR ':NNDataPoint_CON_FUN_MP_CLA:' BRAPH2.WRONG_INPUT '\n' ...
		...
		function check_out = existsTag(tag)
			...
			nndatapoint_con_fun_mp_cla_tag_list = cellfun(@(x) NNDataPoint_CON_FUN_MP_CLA.getPropTag(x), num2cell(NNDataPoint_CON_FUN_MP_CLA.getProps()), 'UniformOutput', false);
			check = any(strcmp(tag, nndatapoint_con_fun_mp_cla_tag_list));
		...
		function prop = getPropProp(pointer)
			...
			if ischar(pointer)
				nndatapoint_con_fun_mp_cla_tag_list = cellfun(@(x) NNDataPoint_CON_FUN_MP_CLA.getPropTag(x), num2cell(NNDataPoint_CON_FUN_MP_CLA.getProps()), 'UniformOutput', false);
				prop = find(strcmp(pointer, nndatapoint_con_fun_mp_cla_tag_list)); % tag = pointer
			else % numeric
				prop = pointer;
			end
		end
		function tag = getPropTag(pointer)
			...
			if ischar(pointer)
				...
				switch prop
					case NNDataPoint_CON_FUN_MP_CLA.SUB
						tag = NNDataPoint_CON_FUN_MP_CLA.SUB_TAG;
					case NNDataPoint_CON_FUN_MP_CLA.TARGET_IDS
						tag = NNDataPoint_CON_FUN_MP_CLA.TARGET_IDS_TAG;
					otherwise
		...
		function prop_category = getPropCategory(pointer)
			...
			switch prop
				case NNDataPoint_CON_FUN_MP_CLA.SUB
					prop_category = NNDataPoint_CON_FUN_MP_CLA.SUB_CATEGORY;
				case NNDataPoint_CON_FUN_MP_CLA.TARGET_IDS
					prop_category = NNDataPoint_CON_FUN_MP_CLA.TARGET_IDS_CATEGORY;
				otherwise
		...
		function prop_format = getPropFormat(pointer)
			...
			switch prop
				case NNDataPoint_CON_FUN_MP_CLA.SUB
					prop_format = NNDataPoint_CON_FUN_MP_CLA.SUB_FORMAT;
				case NNDataPoint_CON_FUN_MP_CLA.TARGET_IDS
					prop_format = NNDataPoint_CON_FUN_MP_CLA.TARGET_IDS_FORMAT;
				otherwise
		...
		function prop_description = getPropDescription(pointer)
			...
			switch prop
				case NNDataPoint_CON_FUN_MP_CLA.SUB
					prop_description = 'SUB (data, item) is a subject with connectivity-functional multiplex data.';
				case NNDataPoint_CON_FUN_MP_CLA.TARGET_IDS
					prop_description = 'TARGET_IDS (parameter, stringlist) is a list of variable-of-interest IDs to be used as the class targets.';
				case NNDataPoint_CON_FUN_MP_CLA.NAME
					prop_description = 'NAME (constant, string) is the name of a data point for classification with connectivity-functional multiplex data.';
				case NNDataPoint_CON_FUN_MP_CLA.DESCRIPTION
					prop_description = 'DESCRIPTION (constant, string) is the description of a data point for classification with connectivity-functional multiplex data.';
				case NNDataPoint_CON_FUN_MP_CLA.TEMPLATE
					prop_description = 'TEMPLATE (parameter, item) is the template of a data point for classification with connectivity-functional multiplex data.';
				case NNDataPoint_CON_FUN_MP_CLA.ID
					prop_description = 'ID (data, string) is a few-letter code for a data point for classification with connectivity-functional multiplex data.';
				case NNDataPoint_CON_FUN_MP_CLA.LABEL
					prop_description = 'LABEL (metadata, string) is an extended label of a data point for classification with connectivity-functional multiplex data.';
				case NNDataPoint_CON_FUN_MP_CLA.NOTES
					prop_description = 'NOTES (metadata, string) are some specific notes about a data point for classification with connectivity-functional multiplex data.';
				case NNDataPoint_CON_FUN_MP_CLA.INPUT
					prop_description = 'INPUT (result, cell) is the input value for this data point.';
				case NNDataPoint_CON_FUN_MP_CLA.TARGET
		...
		function prop_default = getPropDefault(pointer)
			...
			switch prop
				case NNDataPoint_CON_FUN_MP_CLA.SUB
					prop_default = Format.getFormatDefault(Format.ITEM, NNDataPoint_CON_FUN_MP_CLA.getPropSettings(prop));
				case NNDataPoint_CON_FUN_MP_CLA.TARGET_IDS
					prop_default = Format.getFormatDefault(Format.STRINGLIST, NNDataPoint_CON_FUN_MP_CLA.getPropSettings(prop));
				case NNDataPoint_CON_FUN_MP_CLA.NAME
	...
	methods (Static) % checkProp
		function prop_check = checkProp(pointer, value)
			...
			%  Error id: €BRAPH2.STR€:NNDataPoint_CON_FUN_MP_CLA:€BRAPH2.WRONG_INPUT€
			% 
			% Alternative forms to call this method are (POINTER = PROP or TAG):
			...
			switch prop
				case NNDataPoint_CON_FUN_MP_CLA.SUB % __NNDataPoint_CON_FUN_MP_CLA.SUB__
					check = Format.checkFormat(Format.ITEM, value, NNDataPoint_CON_FUN_MP_CLA.getPropSettings(prop));
				case NNDataPoint_CON_FUN_MP_CLA.TARGET_IDS % __NNDataPoint_CON_FUN_MP_CLA.TARGET_IDS__
					check = Format.checkFormat(Format.STRINGLIST, value, NNDataPoint_CON_FUN_MP_CLA.getPropSettings(prop));
				case NNDataPoint_CON_FUN_MP_CLA.TEMPLATE % __NNDataPoint_CON_FUN_MP_CLA.TEMPLATE__
					check = Format.checkFormat(Format.ITEM, value, NNDataPoint_CON_FUN_MP_CLA.getPropSettings(prop));
				otherwise
	...
	methods (Access=protected) % calculate value
		function value = calculateValue(dp, prop, varargin)
			...
			switch prop
				case NNDataPoint_CON_FUN_MP_CLA.INPUT % __NNDataPoint_CON_FUN_MP_CLA.INPUT__
					rng_settings_ = rng(); rng(dp.getPropSeed(NNDataPoint_CON_FUN_MP_CLA.INPUT), 'twister')
				...
				case NNDataPoint_CON_FUN_MP_CLA.TARGET % __NNDataPoint_CON_FUN_MP_CLA.TARGET__
					rng_settings_ = rng(); rng(dp.getPropSeed(NNDataPoint_CON_FUN_MP_CLA.TARGET), 'twister')
