classdef SubjectPP_VOIDict < PanelProp
	...
	%  tostring - string with information about the VOI plot
	%  disp - displays information about the VOI plot
	%  tree - displays the tree of the VOI plot
	%
	% SubjectPP_VOIDict methods (miscellanea):
	%  getNoValue - returns a pointer to a persistent instance of NoValue
	%               Use it as Element.getNoValue()
	%  getCallback - returns the callback to a property
	%  isequal - determines whether two VOI plot are equal (values, locked)
	%  getElementList - returns a list with all subelements
	%  copy - copies the VOI plot
	%
	% SubjectPP_VOIDict methods (save/load, Static):
	%  save - saves BRAPH2 VOI plot as b2 file
	%  load - loads a BRAPH2 VOI plot from a b2 file
	%
	% SubjectPP_VOIDict method (JSON encode):
	%  encodeJSON - returns a JSON string encoding the VOI plot
	%
	% SubjectPP_VOIDict method (JSON decode, Static):
	%   decodeJSON - returns a JSON string encoding the VOI plot
	%
	% SubjectPP_VOIDict methods (inspection, Static):
	%  getClass - returns the class of the VOI plot
	%  getSubclasses - returns all subclasses of SubjectPP_VOIDict
	%  getProps - returns the property list of the VOI plot
	%  getPropNumber - returns the property number of the VOI plot
	...
	methods % constructor
		function pr = SubjectPP_VOIDict(varargin)
			%SubjectPP_VOIDict() creates a VOI plot.
			%
	...
	methods (Static) % inspection
		function pr_class = getClass()
			...
			%  CLASS = PR.GETCLASS() returns the class of the VOI plot PR.
		...
		function subclass_list = getSubclasses()
			%GETSUBCLASSES returns all subclasses of the VOI plot.
			%
			% LIST = SubjectPP_VOIDict.GETSUBCLASSES() returns all subclasses of 'SubjectPP_VOIDict'.
			%
			% Alternative forms to call this method are:
			%  LIST = PR.GETSUBCLASSES() returns all subclasses of the VOI plot PR.
		...
		function prop_list = getProps(category)
			%GETPROPS returns the property list of VOI plot.
			%
			% PROPS = SubjectPP_VOIDict.GETPROPS() returns the property list of VOI plot
			%  as a row vector.
			%
			...
			%
			% Alternative forms to call this method are:
			%  PROPS = PR.GETPROPS([CATEGORY]) returns the property list of the VOI plot PR.
		...
		function prop_number = getPropNumber(varargin)
			%GETPROPNUMBER returns the property number of VOI plot.
			%
			% N = SubjectPP_VOIDict.GETPROPNUMBER() returns the property number of VOI plot.
			%
			% N = SubjectPP_VOIDict.GETPROPNUMBER(CATEGORY) returns the property number of VOI plot
			%  of category CATEGORY
			%
			% Alternative forms to call this method are:
			%  N = PR.GETPROPNUMBER([CATEGORY]) returns the property number of the VOI plot PR.
		...
		function check_out = existsProp(prop)
			%EXISTSPROP checks whether property exists in VOI plot/error.
			%
		...
		function check_out = existsTag(tag)
			%EXISTSTAG checks whether tag exists in VOI plot/error.
			%
		...
		function prop_description = getPropDescription(pointer)
			...
			switch prop
				...
				case SubjectPP_VOIDict.NAME
					prop_description = 'NAME (constant, string) is the name of the VOI plot.';
				case SubjectPP_VOIDict.DESCRIPTION
					prop_description = 'DESCRIPTION (constant, string) is the description of the VOI plot.';
				case SubjectPP_VOIDict.TEMPLATE
					prop_description = 'TEMPLATE (parameter, item) is the template of the VOI plot.';
				case SubjectPP_VOIDict.ID
					prop_description = 'ID (data, string) is a few-letter code for the VOI plot.';
				case SubjectPP_VOIDict.LABEL
					prop_description = 'LABEL (metadata, string) is an extended label of the VOI plot.';
				case SubjectPP_VOIDict.NOTES
					prop_description = 'NOTES (metadata, string) are some specific notes about the VOI plot.';
		...
		function prop_default = getPropDefault(pointer)
			...
			switch prop
				...
				case SubjectPP_VOIDict.DESCRIPTION
					prop_default = 'SubjectPP_VOIDict plots a VOI (variables of interest) dictionary using numeric editfields and drop-down lists.';
				case SubjectPP_VOIDict.TEMPLATE
