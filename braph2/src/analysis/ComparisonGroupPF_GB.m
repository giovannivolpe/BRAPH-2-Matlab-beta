classdef ComparisonGroupPF_GB < ComparisonGroupPF
	...
	methods (Static) % inspection
		...
		function subclass_list = getSubclasses()
			...
			subclass_list = subclasses('ComparisonGroupPF_GB', [], [], true);
		end
		...
		function prop_number = getPropNumber(varargin)
			...
			prop_number = numel(ComparisonGroupPF_GB.getProps(varargin{:}));
		end
		function check_out = existsProp(prop)
			...
			check = any(prop == ComparisonGroupPF_GB.getProps());
			
			if nargout == 1
				check_out = check;
			elseif ~check
				error( ...
					[BRAPH2.STR ':ComparisonGroupPF_GB:' BRAPH2.WRONG_INPUT], ...
					[BRAPH2.STR ':ComparisonGroupPF_GB:' BRAPH2.WRONG_INPUT '\n' ...
		...
		function check_out = existsTag(tag)
			...
			comparisongrouppf_gb_tag_list = cellfun(@(x) ComparisonGroupPF_GB.getPropTag(x), num2cell(ComparisonGroupPF_GB.getProps()), 'UniformOutput', false);
			check = any(strcmp(tag, comparisongrouppf_gb_tag_list));
		...
		function prop = getPropProp(pointer)
			...
			if ischar(pointer)
				comparisongrouppf_gb_tag_list = cellfun(@(x) ComparisonGroupPF_GB.getPropTag(x), num2cell(ComparisonGroupPF_GB.getProps()), 'UniformOutput', false);
				prop = find(strcmp(pointer, comparisongrouppf_gb_tag_list)); % tag = pointer
			else % numeric
				prop = pointer;
			end
		end
		...
		function prop_description = getPropDescription(pointer)
			...
			switch prop
				...
				case ComparisonGroupPF_GB.SETUP
		...
		function prop_default = getPropDefault(pointer)
			...
			switch prop
				...
				case ComparisonGroupPF_GB.TEMPLATE
					prop_default = Format.getFormatDefault(Format.ITEM, ComparisonGroupPF_GB.getPropSettings(prop));
				case ComparisonGroupPF_GB.ID
	...
	methods (Static) % checkProp
		function prop_check = checkProp(pointer, value)
			...
			%  Error id: €BRAPH2.STR€:ComparisonGroupPF_GB:€BRAPH2.WRONG_INPUT€
			% 
			% Alternative forms to call this method are (POINTER = PROP or TAG):
			...
			switch prop
				case ComparisonGroupPF_GB.TEMPLATE % __ComparisonGroupPF_GB.TEMPLATE__
					check = Format.checkFormat(Format.ITEM, value, ComparisonGroupPF_GB.getPropSettings(prop));
				otherwise
	...
	methods (Access=protected) % calculate value
		function value = calculateValue(pf, prop, varargin)
			...
			switch prop
				case ComparisonGroupPF_GB.SETUP % __ComparisonGroupPF_GB.SETUP__
					%%%__WARN_TBI__
					value = [];
