classdef ComparisonGroupPF_GU < ComparisonGroupPF
	...
	methods (Static) % inspection
		...
		function subclass_list = getSubclasses()
			...
			subclass_list = subclasses('ComparisonGroupPF_GU', [], [], true);
		end
		...
		function prop_number = getPropNumber(varargin)
			...
			prop_number = numel(ComparisonGroupPF_GU.getProps(varargin{:}));
		end
		function check_out = existsProp(prop)
			...
			check = any(prop == ComparisonGroupPF_GU.getProps());
			
			if nargout == 1
				check_out = check;
			elseif ~check
				error( ...
					[BRAPH2.STR ':ComparisonGroupPF_GU:' BRAPH2.WRONG_INPUT], ...
					[BRAPH2.STR ':ComparisonGroupPF_GU:' BRAPH2.WRONG_INPUT '\n' ...
		...
		function check_out = existsTag(tag)
			...
			comparisongrouppf_gu_tag_list = cellfun(@(x) ComparisonGroupPF_GU.getPropTag(x), num2cell(ComparisonGroupPF_GU.getProps()), 'UniformOutput', false);
			check = any(strcmp(tag, comparisongrouppf_gu_tag_list));
		...
		function prop = getPropProp(pointer)
			...
			if ischar(pointer)
				comparisongrouppf_gu_tag_list = cellfun(@(x) ComparisonGroupPF_GU.getPropTag(x), num2cell(ComparisonGroupPF_GU.getProps()), 'UniformOutput', false);
				prop = find(strcmp(pointer, comparisongrouppf_gu_tag_list)); % tag = pointer
			else % numeric
				prop = pointer;
			end
		end
		...
		function prop_description = getPropDescription(pointer)
			...
			switch prop
				...
				case ComparisonGroupPF_GU.SETUP
		...
		function prop_default = getPropDefault(pointer)
			...
			switch prop
				...
				case ComparisonGroupPF_GU.TEMPLATE
					prop_default = Format.getFormatDefault(Format.ITEM, ComparisonGroupPF_GU.getPropSettings(prop));
				case ComparisonGroupPF_GU.ID
	...
	methods (Static) % checkProp
		function prop_check = checkProp(pointer, value)
			...
			%  Error id: €BRAPH2.STR€:ComparisonGroupPF_GU:€BRAPH2.WRONG_INPUT€
			% 
			% Alternative forms to call this method are (POINTER = PROP or TAG):
			...
			switch prop
				case ComparisonGroupPF_GU.TEMPLATE % __ComparisonGroupPF_GU.TEMPLATE__
					check = Format.checkFormat(Format.ITEM, value, ComparisonGroupPF_GU.getPropSettings(prop));
				otherwise
	...
	methods (Access=protected) % calculate value
		function value = calculateValue(pf, prop, varargin)
			...
			switch prop
				case ComparisonGroupPF_GU.SETUP % __ComparisonGroupPF_GU.SETUP__
					cp = pf.get('CP');
