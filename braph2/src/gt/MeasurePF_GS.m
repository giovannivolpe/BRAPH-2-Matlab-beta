classdef MeasurePF_GS < MeasurePF
	...
	%  tostring - string with information about the panel figure global superglobal measure
	%  disp - displays information about the panel figure global superglobal measure
	%  tree - displays the tree of the panel figure global superglobal measure
	%
	% MeasurePF_GS methods (miscellanea):
	%  getNoValue - returns a pointer to a persistent instance of NoValue
	%               Use it as Element.getNoValue()
	%  getCallback - returns the callback to a property
	%  isequal - determines whether two panel figure global superglobal measure are equal (values, locked)
	%  getElementList - returns a list with all subelements
	%  copy - copies the panel figure global superglobal measure
	%
	% MeasurePF_GS methods (save/load, Static):
	%  save - saves BRAPH2 panel figure global superglobal measure as b2 file
	%  load - loads a BRAPH2 panel figure global superglobal measure from a b2 file
	%
	% MeasurePF_GS method (JSON encode):
	%  encodeJSON - returns a JSON string encoding the panel figure global superglobal measure
	%
	% MeasurePF_GS method (JSON decode, Static):
	%   decodeJSON - returns a JSON string encoding the panel figure global superglobal measure
	%
	% MeasurePF_GS methods (inspection, Static):
	%  getClass - returns the class of the panel figure global superglobal measure
	%  getSubclasses - returns all subclasses of MeasurePF_GS
	%  getProps - returns the property list of the panel figure global superglobal measure
	%  getPropNumber - returns the property number of the panel figure global superglobal measure
	...
	methods % constructor
		function pf = MeasurePF_GS(varargin)
			%MeasurePF_GS() creates a panel figure global superglobal measure.
			%
	...
	methods (Static) % inspection
		function pf_class = getClass()
			...
			%  CLASS = PF.GETCLASS() returns the class of the panel figure global superglobal measure PF.
		...
		function subclass_list = getSubclasses()
			%GETSUBCLASSES returns all subclasses of the panel figure global superglobal measure.
			%
			% LIST = MeasurePF_GS.GETSUBCLASSES() returns all subclasses of 'MeasurePF_GS'.
			%
			% Alternative forms to call this method are:
			%  LIST = PF.GETSUBCLASSES() returns all subclasses of the panel figure global superglobal measure PF.
		...
		function prop_list = getProps(category)
			%GETPROPS returns the property list of panel figure global superglobal measure.
			%
			% PROPS = MeasurePF_GS.GETPROPS() returns the property list of panel figure global superglobal measure
			%  as a row vector.
			%
			...
			%
			% Alternative forms to call this method are:
			%  PROPS = PF.GETPROPS([CATEGORY]) returns the property list of the panel figure global superglobal measure PF.
		...
		function prop_number = getPropNumber(varargin)
			%GETPROPNUMBER returns the property number of panel figure global superglobal measure.
			%
			% N = MeasurePF_GS.GETPROPNUMBER() returns the property number of panel figure global superglobal measure.
			%
			% N = MeasurePF_GS.GETPROPNUMBER(CATEGORY) returns the property number of panel figure global superglobal measure
			%  of category CATEGORY
			%
			% Alternative forms to call this method are:
			%  N = PF.GETPROPNUMBER([CATEGORY]) returns the property number of the panel figure global superglobal measure PF.
		...
		function check_out = existsProp(prop)
			%EXISTSPROP checks whether property exists in panel figure global superglobal measure/error.
			%
		...
		function check_out = existsTag(tag)
			%EXISTSTAG checks whether tag exists in panel figure global superglobal measure/error.
			%
		...
		function prop_description = getPropDescription(pointer)
			...
			switch prop
				case MeasurePF_GS.NAME
					prop_description = 'NAME (constant, string) is the name of the panel figure global superglobal measure.';
				case MeasurePF_GS.DESCRIPTION
					prop_description = 'DESCRIPTION (constant, string) is the description of the panel figure global superglobal measure.';
				case MeasurePF_GS.TEMPLATE
					prop_description = 'TEMPLATE (parameter, item) is the template of the panel figure global superglobal measure.';
				case MeasurePF_GS.ID
					prop_description = 'ID (data, string) is a few-letter code for the panel figure global superglobal measure.';
				case MeasurePF_GS.LABEL
					prop_description = 'LABEL (metadata, string) is an extended label of the panel figure global superglobal measure.';
				case MeasurePF_GS.NOTES
					prop_description = 'NOTES (metadata, string) are some specific notes about the panel figure global superglobal measure.';
		...
		function prop_default = getPropDefault(pointer)
			...
			switch prop
				...
				case MeasurePF_GS.DESCRIPTION
					prop_default = 'MeasurePF_GS manages the basic functionalities to plot of a global superglobal measure.';
				case MeasurePF_GS.TEMPLATE
