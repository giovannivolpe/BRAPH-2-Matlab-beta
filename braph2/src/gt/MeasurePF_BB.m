classdef MeasurePF_BB < MeasurePF
	...
	%  tostring - string with information about the panel figure binodal bilayer measure
	%  disp - displays information about the panel figure binodal bilayer measure
	%  tree - displays the tree of the panel figure binodal bilayer measure
	%
	% MeasurePF_BB methods (miscellanea):
	%  getNoValue - returns a pointer to a persistent instance of NoValue
	%               Use it as Element.getNoValue()
	%  getCallback - returns the callback to a property
	%  isequal - determines whether two panel figure binodal bilayer measure are equal (values, locked)
	%  getElementList - returns a list with all subelements
	%  copy - copies the panel figure binodal bilayer measure
	%
	% MeasurePF_BB methods (save/load, Static):
	%  save - saves BRAPH2 panel figure binodal bilayer measure as b2 file
	%  load - loads a BRAPH2 panel figure binodal bilayer measure from a b2 file
	%
	% MeasurePF_BB method (JSON encode):
	%  encodeJSON - returns a JSON string encoding the panel figure binodal bilayer measure
	%
	% MeasurePF_BB method (JSON decode, Static):
	%   decodeJSON - returns a JSON string encoding the panel figure binodal bilayer measure
	%
	% MeasurePF_BB methods (inspection, Static):
	%  getClass - returns the class of the panel figure binodal bilayer measure
	%  getSubclasses - returns all subclasses of MeasurePF_BB
	%  getProps - returns the property list of the panel figure binodal bilayer measure
	%  getPropNumber - returns the property number of the panel figure binodal bilayer measure
	...
	methods % constructor
		function pf = MeasurePF_BB(varargin)
			%MeasurePF_BB() creates a panel figure binodal bilayer measure.
			%
	...
	methods (Static) % inspection
		function pf_class = getClass()
			...
			%  CLASS = PF.GETCLASS() returns the class of the panel figure binodal bilayer measure PF.
		...
		function subclass_list = getSubclasses()
			%GETSUBCLASSES returns all subclasses of the panel figure binodal bilayer measure.
			%
			% LIST = MeasurePF_BB.GETSUBCLASSES() returns all subclasses of 'MeasurePF_BB'.
			%
			% Alternative forms to call this method are:
			%  LIST = PF.GETSUBCLASSES() returns all subclasses of the panel figure binodal bilayer measure PF.
		...
		function prop_list = getProps(category)
			%GETPROPS returns the property list of panel figure binodal bilayer measure.
			%
			% PROPS = MeasurePF_BB.GETPROPS() returns the property list of panel figure binodal bilayer measure
			%  as a row vector.
			%
			...
			%
			% Alternative forms to call this method are:
			%  PROPS = PF.GETPROPS([CATEGORY]) returns the property list of the panel figure binodal bilayer measure PF.
		...
		function prop_number = getPropNumber(varargin)
			%GETPROPNUMBER returns the property number of panel figure binodal bilayer measure.
			%
			% N = MeasurePF_BB.GETPROPNUMBER() returns the property number of panel figure binodal bilayer measure.
			%
			% N = MeasurePF_BB.GETPROPNUMBER(CATEGORY) returns the property number of panel figure binodal bilayer measure
			%  of category CATEGORY
			%
			% Alternative forms to call this method are:
			%  N = PF.GETPROPNUMBER([CATEGORY]) returns the property number of the panel figure binodal bilayer measure PF.
		...
		function check_out = existsProp(prop)
			%EXISTSPROP checks whether property exists in panel figure binodal bilayer measure/error.
			%
		...
		function check_out = existsTag(tag)
			%EXISTSTAG checks whether tag exists in panel figure binodal bilayer measure/error.
			%
		...
		function prop_description = getPropDescription(pointer)
			...
			switch prop
				...
				case MeasurePF_BB.NAME
					prop_description = 'NAME (constant, string) is the name of the panel figure binodal bilayer measure.';
				case MeasurePF_BB.DESCRIPTION
					prop_description = 'DESCRIPTION (constant, string) is the description of the panel figure binodal bilayer measure.';
				case MeasurePF_BB.TEMPLATE
					prop_description = 'TEMPLATE (parameter, item) is the template of the panel figure binodal bilayer measure.';
				case MeasurePF_BB.ID
					prop_description = 'ID (data, string) is a few-letter code for the panel figure binodal bilayer measure.';
				case MeasurePF_BB.LABEL
					prop_description = 'LABEL (metadata, string) is an extended label of the panel figure binodal bilayer measure.';
				case MeasurePF_BB.NOTES
					prop_description = 'NOTES (metadata, string) are some specific notes about the panel figure binodal bilayer measure.';
		...
		function prop_default = getPropDefault(pointer)
			...
			switch prop
				...
				case MeasurePF_BB.DESCRIPTION
					prop_default = 'MeasurePF_BB manages the basic functionalities to plot of a binodal bilayer measure.';
				case MeasurePF_BB.TEMPLATE
