classdef MeasurePF_NU < MeasurePF
	...
	%  tostring - string with information about the panel figure nodal unilayer measure
	%  disp - displays information about the panel figure nodal unilayer measure
	%  tree - displays the tree of the panel figure nodal unilayer measure
	%
	% MeasurePF_NU methods (miscellanea):
	%  getNoValue - returns a pointer to a persistent instance of NoValue
	%               Use it as Element.getNoValue()
	%  getCallback - returns the callback to a property
	%  isequal - determines whether two panel figure nodal unilayer measure are equal (values, locked)
	%  getElementList - returns a list with all subelements
	%  copy - copies the panel figure nodal unilayer measure
	%
	% MeasurePF_NU methods (save/load, Static):
	%  save - saves BRAPH2 panel figure nodal unilayer measure as b2 file
	%  load - loads a BRAPH2 panel figure nodal unilayer measure from a b2 file
	%
	% MeasurePF_NU method (JSON encode):
	%  encodeJSON - returns a JSON string encoding the panel figure nodal unilayer measure
	%
	% MeasurePF_NU method (JSON decode, Static):
	%   decodeJSON - returns a JSON string encoding the panel figure nodal unilayer measure
	%
	% MeasurePF_NU methods (inspection, Static):
	%  getClass - returns the class of the panel figure nodal unilayer measure
	%  getSubclasses - returns all subclasses of MeasurePF_NU
	%  getProps - returns the property list of the panel figure nodal unilayer measure
	%  getPropNumber - returns the property number of the panel figure nodal unilayer measure
	...
	methods % constructor
		function pf = MeasurePF_NU(varargin)
			%MeasurePF_NU() creates a panel figure nodal unilayer measure.
			%
	...
	methods (Static) % inspection
		function pf_class = getClass()
			...
			%  CLASS = PF.GETCLASS() returns the class of the panel figure nodal unilayer measure PF.
		...
		function subclass_list = getSubclasses()
			%GETSUBCLASSES returns all subclasses of the panel figure nodal unilayer measure.
			%
			% LIST = MeasurePF_NU.GETSUBCLASSES() returns all subclasses of 'MeasurePF_NU'.
			%
			% Alternative forms to call this method are:
			%  LIST = PF.GETSUBCLASSES() returns all subclasses of the panel figure nodal unilayer measure PF.
		...
		function prop_list = getProps(category)
			%GETPROPS returns the property list of panel figure nodal unilayer measure.
			%
			% PROPS = MeasurePF_NU.GETPROPS() returns the property list of panel figure nodal unilayer measure
			%  as a row vector.
			%
			...
			%
			% Alternative forms to call this method are:
			%  PROPS = PF.GETPROPS([CATEGORY]) returns the property list of the panel figure nodal unilayer measure PF.
		...
		function prop_number = getPropNumber(varargin)
			%GETPROPNUMBER returns the property number of panel figure nodal unilayer measure.
			%
			% N = MeasurePF_NU.GETPROPNUMBER() returns the property number of panel figure nodal unilayer measure.
			%
			% N = MeasurePF_NU.GETPROPNUMBER(CATEGORY) returns the property number of panel figure nodal unilayer measure
			%  of category CATEGORY
			%
			% Alternative forms to call this method are:
			%  N = PF.GETPROPNUMBER([CATEGORY]) returns the property number of the panel figure nodal unilayer measure PF.
		...
		function check_out = existsProp(prop)
			%EXISTSPROP checks whether property exists in panel figure nodal unilayer measure/error.
			%
		...
		function check_out = existsTag(tag)
			%EXISTSTAG checks whether tag exists in panel figure nodal unilayer measure/error.
			%
		...
		function prop_description = getPropDescription(pointer)
			...
			switch prop
				...
				case MeasurePF_NU.NAME
					prop_description = 'NAME (constant, string) is the name of the panel figure nodal unilayer measure.';
				case MeasurePF_NU.DESCRIPTION
					prop_description = 'DESCRIPTION (constant, string) is the description of the panel figure nodal unilayer measure.';
				case MeasurePF_NU.TEMPLATE
					prop_description = 'TEMPLATE (parameter, item) is the template of the panel figure nodal unilayer measure.';
				case MeasurePF_NU.ID
					prop_description = 'ID (data, string) is a few-letter code for the panel figure nodal unilayer measure.';
				case MeasurePF_NU.LABEL
					prop_description = 'LABEL (metadata, string) is an extended label of the panel figure nodal unilayer measure.';
				case MeasurePF_NU.NOTES
					prop_description = 'NOTES (metadata, string) are some specific notes about the panel figure nodal unilayer measure.';
		...
		function prop_default = getPropDefault(pointer)
			...
			switch prop
				...
				case MeasurePF_NU.DESCRIPTION
					prop_default = 'MeasurePF_NU manages the basic functionalities to plot of a nodal unilayer measure.';
				case MeasurePF_NU.TEMPLATE
