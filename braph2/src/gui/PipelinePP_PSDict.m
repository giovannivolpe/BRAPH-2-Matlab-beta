		...
		function prop_description = getPropDescription(pointer)
			...
			switch prop
				...
				case PipelinePP_PSDict.NAME
					prop_description = 'NAME (constant, string) is the name of the pipeline plot.';
				case PipelinePP_PSDict.DESCRIPTION
					prop_description = 'DESCRIPTION (constant, string) is the description of the pipeline plot.';
				case PipelinePP_PSDict.TEMPLATE
					prop_description = 'TEMPLATE (parameter, item) is the template of the pipeline plot.';
				case PipelinePP_PSDict.ID
					prop_description = 'ID (data, string) is a few-letter code for the pipeline plot.';
				case PipelinePP_PSDict.LABEL
					prop_description = 'LABEL (metadata, string) is an extended label of the pipeline plot.';
		...
		function value = calculateValue(pr, prop, varargin)
			...
			switch prop
				...
				case PipelinePP_PSDict.PC_CLEAR_BTN % __PipelinePP_PSDict.PC_CLEAR_BTN__
					...
					for i = 1:1:length(handles)
					    if check_graphics(handles{i}, 'uibutton') && strcmpi(handles{i}.get('Tag'), ['clear ' int2str(sec) '.' int2str(cl)]) % for Tag see HANDLES
					        value = handles{i}; % output value
					        break
					    end
					end
