classdef PanelPropNet < PanelProp
	...
	%  tostring - string with information about the panel property net
	%  disp - displays information about the panel property net
	%  tree - displays the tree of the panel property net
	%
	% PanelPropNet methods (miscellanea):
	%  getNoValue - returns a pointer to a persistent instance of NoValue
	%               Use it as Element.getNoValue()
	%  getCallback - returns the callback to a property
	%  isequal - determines whether two panel property net are equal (values, locked)
	%  getElementList - returns a list with all subelements
	%  copy - copies the panel property net
	%
	% PanelPropNet methods (save/load, Static):
	%  save - saves BRAPH2 panel property net as b2 file
	%  load - loads a BRAPH2 panel property net from a b2 file
	%
	% PanelPropNet method (JSON encode):
	%  encodeJSON - returns a JSON string encoding the panel property net
	%
	% PanelPropNet method (JSON decode, Static):
	%   decodeJSON - returns a JSON string encoding the panel property net
	%
	% PanelPropNet methods (inspection, Static):
	%  getClass - returns the class of the panel property net
	%  getSubclasses - returns all subclasses of PanelPropNet
	%  getProps - returns the property list of the panel property net
	%  getPropNumber - returns the property number of the panel property net
	...
	methods % constructor
		function pr = PanelPropNet(varargin)
			%PanelPropNet() creates a panel property net.
			%
	...
	methods (Static) % inspection
		function pr_class = getClass()
			...
			%  CLASS = PR.GETCLASS() returns the class of the panel property net PR.
		...
		function subclass_list = getSubclasses()
			%GETSUBCLASSES returns all subclasses of the panel property net.
			%
			% LIST = PanelPropNet.GETSUBCLASSES() returns all subclasses of 'PanelPropNet'.
			%
			% Alternative forms to call this method are:
			%  LIST = PR.GETSUBCLASSES() returns all subclasses of the panel property net PR.
		...
		function prop_list = getProps(category)
			%GETPROPS returns the property list of panel property net.
			%
			% PROPS = PanelPropNet.GETPROPS() returns the property list of panel property net
			%  as a row vector.
			%
			...
			%
			% Alternative forms to call this method are:
			%  PROPS = PR.GETPROPS([CATEGORY]) returns the property list of the panel property net PR.
		...
		function prop_number = getPropNumber(varargin)
			%GETPROPNUMBER returns the property number of panel property net.
			%
			% N = PanelPropNet.GETPROPNUMBER() returns the property number of panel property net.
			%
			% N = PanelPropNet.GETPROPNUMBER(CATEGORY) returns the property number of panel property net
			%  of category CATEGORY
			%
			% Alternative forms to call this method are:
			%  N = PR.GETPROPNUMBER([CATEGORY]) returns the property number of the panel property net PR.
		...
		function check_out = existsProp(prop)
			%EXISTSPROP checks whether property exists in panel property net/error.
			%
		...
		function check_out = existsTag(tag)
			%EXISTSTAG checks whether tag exists in panel property net/error.
			%
		...
		function prop_default = getPropDefault(pointer)
			...
			switch prop
				...
				case PanelPropNet.DESCRIPTION
					prop_default = 'PanelPropNet plots the panel for a NET property with a button. It works for all categories.';
				case PanelPropNet.TEMPLATE
