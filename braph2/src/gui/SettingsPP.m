classdef SettingsPP < PanelProp
	...
	%  tostring - string with information about the settings panel
	%  disp - displays information about the settings panel
	%  tree - displays the tree of the settings panel
	%
	% SettingsPP methods (miscellanea):
	%  getNoValue - returns a pointer to a persistent instance of NoValue
	%               Use it as Element.getNoValue()
	%  getCallback - returns the callback to a property
	%  isequal - determines whether two settings panel are equal (values, locked)
	%  getElementList - returns a list with all subelements
	%  copy - copies the settings panel
	%
	% SettingsPP methods (save/load, Static):
	%  save - saves BRAPH2 settings panel as b2 file
	%  load - loads a BRAPH2 settings panel from a b2 file
	%
	% SettingsPP method (JSON encode):
	%  encodeJSON - returns a JSON string encoding the settings panel
	%
	% SettingsPP method (JSON decode, Static):
	%   decodeJSON - returns a JSON string encoding the settings panel
	%
	% SettingsPP methods (inspection, Static):
	%  getClass - returns the class of the settings panel
	%  getSubclasses - returns all subclasses of SettingsPP
	%  getProps - returns the property list of the settings panel
	%  getPropNumber - returns the property number of the settings panel
	...
	methods % constructor
		function pr = SettingsPP(varargin)
			%SettingsPP() creates a settings panel.
			%
	...
	methods (Static) % inspection
		function pr_class = getClass()
			...
			%  CLASS = PR.GETCLASS() returns the class of the settings panel PR.
		...
		function subclass_list = getSubclasses()
			%GETSUBCLASSES returns all subclasses of the settings panel.
			%
			% LIST = SettingsPP.GETSUBCLASSES() returns all subclasses of 'SettingsPP'.
			%
			% Alternative forms to call this method are:
			%  LIST = PR.GETSUBCLASSES() returns all subclasses of the settings panel PR.
		...
		function prop_list = getProps(category)
			%GETPROPS returns the property list of settings panel.
			%
			% PROPS = SettingsPP.GETPROPS() returns the property list of settings panel
			%  as a row vector.
			%
			...
			%
			% Alternative forms to call this method are:
			%  PROPS = PR.GETPROPS([CATEGORY]) returns the property list of the settings panel PR.
		...
		function prop_number = getPropNumber(varargin)
			%GETPROPNUMBER returns the property number of settings panel.
			%
			% N = SettingsPP.GETPROPNUMBER() returns the property number of settings panel.
			%
			% N = SettingsPP.GETPROPNUMBER(CATEGORY) returns the property number of settings panel
			%  of category CATEGORY
			%
			% Alternative forms to call this method are:
			%  N = PR.GETPROPNUMBER([CATEGORY]) returns the property number of the settings panel PR.
		...
		function check_out = existsProp(prop)
			%EXISTSPROP checks whether property exists in settings panel/error.
			%
		...
		function check_out = existsTag(tag)
			%EXISTSTAG checks whether tag exists in settings panel/error.
			%
		...
		function prop_description = getPropDescription(pointer)
			...
			switch prop
				case SettingsPP.NAME
					prop_description = 'NAME (constant, string) is the name of the settings panel.';
				case SettingsPP.DESCRIPTION
					prop_description = 'DESCRIPTION (constant, string) is the description of the settings panel.';
				case SettingsPP.TEMPLATE
					prop_description = 'TEMPLATE (parameter, item) is the template of the settings panel.';
				case SettingsPP.ID
					prop_description = 'ID (data, string) is a few-letter code for the settings panel.';
				case SettingsPP.LABEL
					prop_description = 'LABEL (metadata, string) is an extended label of the settings panel.';
				case SettingsPP.NOTES
					prop_description = 'NOTES (metadata, string) are some specific notes about the settings panel.';
				case SettingsPP.EL
		...
		function prop_default = getPropDefault(pointer)
			...
			switch prop
				...
				case SettingsPP.DESCRIPTION
					prop_default = 'SettingsPP is the base element for the panels of the Settings. In particular, it provides the updated PanelProp listeners.';
				case SettingsPP.TEMPLATE
