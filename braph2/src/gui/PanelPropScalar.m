classdef PanelPropScalar < PanelProp
	...
	%  tostring - string with information about the panel property scalar
	%  disp - displays information about the panel property scalar
	%  tree - displays the tree of the panel property scalar
	%
	% PanelPropScalar methods (miscellanea):
	%  getNoValue - returns a pointer to a persistent instance of NoValue
	%               Use it as Element.getNoValue()
	%  getCallback - returns the callback to a property
	%  isequal - determines whether two panel property scalar are equal (values, locked)
	%  getElementList - returns a list with all subelements
	%  copy - copies the panel property scalar
	%
	% PanelPropScalar methods (save/load, Static):
	%  save - saves BRAPH2 panel property scalar as b2 file
	%  load - loads a BRAPH2 panel property scalar from a b2 file
	%
	% PanelPropScalar method (JSON encode):
	%  encodeJSON - returns a JSON string encoding the panel property scalar
	%
	% PanelPropScalar method (JSON decode, Static):
	%   decodeJSON - returns a JSON string encoding the panel property scalar
	%
	% PanelPropScalar methods (inspection, Static):
	%  getClass - returns the class of the panel property scalar
	%  getSubclasses - returns all subclasses of PanelPropScalar
	%  getProps - returns the property list of the panel property scalar
	%  getPropNumber - returns the property number of the panel property scalar
	...
	methods % constructor
		function pr = PanelPropScalar(varargin)
			%PanelPropScalar() creates a panel property scalar.
			%
	...
	methods (Static) % inspection
		function pr_class = getClass()
			...
			%  CLASS = PR.GETCLASS() returns the class of the panel property scalar PR.
		...
		function subclass_list = getSubclasses()
			%GETSUBCLASSES returns all subclasses of the panel property scalar.
			%
			% LIST = PanelPropScalar.GETSUBCLASSES() returns all subclasses of 'PanelPropScalar'.
			%
			% Alternative forms to call this method are:
			%  LIST = PR.GETSUBCLASSES() returns all subclasses of the panel property scalar PR.
		...
		function prop_list = getProps(category)
			%GETPROPS returns the property list of panel property scalar.
			%
			% PROPS = PanelPropScalar.GETPROPS() returns the property list of panel property scalar
			%  as a row vector.
			%
			...
			%
			% Alternative forms to call this method are:
			%  PROPS = PR.GETPROPS([CATEGORY]) returns the property list of the panel property scalar PR.
		...
		function prop_number = getPropNumber(varargin)
			%GETPROPNUMBER returns the property number of panel property scalar.
			%
			% N = PanelPropScalar.GETPROPNUMBER() returns the property number of panel property scalar.
			%
			% N = PanelPropScalar.GETPROPNUMBER(CATEGORY) returns the property number of panel property scalar
			%  of category CATEGORY
			%
			% Alternative forms to call this method are:
			%  N = PR.GETPROPNUMBER([CATEGORY]) returns the property number of the panel property scalar PR.
		...
		function check_out = existsProp(prop)
			%EXISTSPROP checks whether property exists in panel property scalar/error.
			%
		...
		function check_out = existsTag(tag)
			%EXISTSTAG checks whether tag exists in panel property scalar/error.
			%
		...
		function prop_default = getPropDefault(pointer)
			...
			switch prop
				...
				case PanelPropScalar.DESCRIPTION
					prop_default = 'PanelPropScalar plots the panel for a SCALAR property with a numeric edit field. It works for all categories.';
				case PanelPropScalar.TEMPLATE
