classdef Panel < ConcreteElement
	...
	%  <strong>1</strong> <strong>NAME</strong> 	NAME (constant, string) is the name of the panel.
	%  <strong>2</strong> <strong>DESCRIPTION</strong> 	DESCRIPTION (constant, string) is the description of the panel.
	%  <strong>3</strong> <strong>TEMPLATE</strong> 	TEMPLATE (parameter, item) is the template of the panel.
	%  <strong>4</strong> <strong>ID</strong> 	ID (data, string) is a few-letter code for the panel.
	%  <strong>5</strong> <strong>LABEL</strong> 	LABEL (metadata, string) is an extended label of the panel.
	%  <strong>6</strong> <strong>NOTES</strong> 	NOTES (metadata, string) are some specific notes about the panel.
	...
	%  <strong>9</strong> <strong>DRAW</strong> 	DRAW (query, logical) draws the panel [accepts uipanel Name-Value pairs].
	...
	properties (Constant) % properties
		...
		H_WAITBAR = 8; %CET: Computational Efficiency Trick
		...
		PARENT = 11; %CET: Computational Efficiency Trick
		PARENT_TAG = 'PARENT';
		PARENT_CATEGORY = 9;
		PARENT_FORMAT = 8;
		...
		SHOW = 14; %CET: Computational Efficiency Trick
		SHOW_TAG = 'SHOW';
		SHOW_CATEGORY = 6;
		SHOW_FORMAT = 4;
		
		HIDE = 15; %CET: Computational Efficiency Trick
		HIDE_TAG = 'HIDE';
		HIDE_CATEGORY = 6;
		HIDE_FORMAT = 4;
		
		DELETE = 16; %CET: Computational Efficiency Trick
		DELETE_TAG = 'DELETE';
		DELETE_CATEGORY = 6;
		DELETE_FORMAT = 4;
		
		CLOSE = 17; %CET: Computational Efficiency Trick
		CLOSE_TAG = 'CLOSE';
		CLOSE_CATEGORY = 6;
		CLOSE_FORMAT = 4;
	end
	...
	methods (Static) % inspection
		...
		function subclass_list = getSubclasses()
			...
			subclass_list = { 'Panel'  'AnalyzeEnsemblePP_MeDict'  'AnalyzeGroupPP_G'  'CompareEnsemblePP_CpDict'  'CompareGroupPP_CpDict'  'ComparisonEnsemblePF'  'ComparisonEnsemblePF_BB'  'ComparisonEnsemblePF_BS'  'ComparisonEnsemblePF_BU'  'ComparisonEnsemblePF_GB'  'ComparisonEnsemblePF_GS'  'ComparisonEnsemblePF_GU'  'ComparisonEnsemblePF_NB'  'ComparisonEnsemblePF_NS'  'ComparisonEnsemblePF_NU'  'ComparisonGroupPF'  'ComparisonGroupPF_BB'  'ComparisonGroupPF_BS'  'ComparisonGroupPF_BU'  'ComparisonGroupPF_GB'  'ComparisonGroupPF_GS'  'ComparisonGroupPF_GU'  'ComparisonGroupPF_NB'  'ComparisonGroupPF_NS'  'ComparisonGroupPF_NU'  'BrainAtlasPF'  'BrainSurfacePF'  'SubjectPP_VOIDict'  'GraphAdjPF'  'GraphHistPF'  'GraphPP_MDict'  'LayersPP'  'MeasurePF'  'MeasurePF_BB'  'MeasurePF_BS'  'MeasurePF_BU'  'MeasurePF_GB'  'MeasurePF_GS'  'MeasurePF_GU'  'MeasurePF_NB'  'MeasurePF_NS'  'MeasurePF_NU'  'PanelElement'  'PanelFig'  'PanelProp'  'PanelPropAlpha'  'PanelPropCell'  'PanelPropClass'  'PanelPropClassList'  'PanelPropColor'  'PanelPropIDict'  'PanelPropIDictTable'  'PanelPropItem'  'PanelPropItemList'  'PanelPropLine'  'PanelPropLogical'  'PanelPropMarker'  'PanelPropMatrix'  'PanelPropNet'  'PanelPropOption'  'PanelPropRVectorSmart'  'PanelPropRVectorView'  'PanelPropScalar'  'PanelPropSize'  'PanelPropString'  'PanelPropStringList'  'PanelPropStringTextArea'  'PipelinePP_PSDict'  'SettingsAmbientPP'  'SettingsAreaPP'  'SettingsAxisPP'  'SettingsLinePP'  'SettingsPP'  'SettingsPositionPP'  'SettingsSurfacePP'  'SettingsTextPP' }; %CET: Computational Efficiency Trick
		end
		...
		function check_out = existsTag(tag)
			...
			check = any(strcmp(tag, { 'NAME'  'DESCRIPTION'  'TEMPLATE'  'ID'  'LABEL'  'NOTES'  'WAITBAR'  'H_WAITBAR'  'DRAW'  'DRAWN'  'PARENT'  'BKGCOLOR'  'H'  'SHOW'  'HIDE'  'DELETE'  'CLOSE' })); %CET: Computational Efficiency Trick
		...
		function prop = getPropProp(pointer)
			...
			if ischar(pointer)
				prop = find(strcmp(pointer, { 'NAME'  'DESCRIPTION'  'TEMPLATE'  'ID'  'LABEL'  'NOTES'  'WAITBAR'  'H_WAITBAR'  'DRAW'  'DRAWN'  'PARENT'  'BKGCOLOR'  'H'  'SHOW'  'HIDE'  'DELETE'  'CLOSE' })); % tag = pointer %CET: Computational Efficiency Trick
			else % numeric
				prop = pointer;
			end
		end
		function tag = getPropTag(pointer)
			...
			if ischar(pointer)
				...
				%CET: Computational Efficiency Trick
				panel_tag_list = { 'NAME'  'DESCRIPTION'  'TEMPLATE'  'ID'  'LABEL'  'NOTES'  'WAITBAR'  'H_WAITBAR'  'DRAW'  'DRAWN'  'PARENT'  'BKGCOLOR'  'H'  'SHOW'  'HIDE'  'DELETE'  'CLOSE' };
		...
		function prop_category = getPropCategory(pointer)
			...
			%CET: Computational Efficiency Trick
			panel_category_list = { 1  1  3  4  2  2  9  7  6  6  9  8  7  6  6  6  6 };
		...
		function prop_format = getPropFormat(pointer)
			...
			%CET: Computational Efficiency Trick
			panel_format_list = { 2  2  8  2  2  2  4  18  4  4  8  20  18  4  4  4  4 };
		...
		function prop_description = getPropDescription(pointer)
			...
			%CET: Computational Efficiency Trick
			panel_description_list = { 'NAME (constant, string) is the name of the panel.'  'DESCRIPTION (constant, string) is the description of the panel.'  'TEMPLATE (parameter, item) is the template of the panel.'  'ID (data, string) is a few-letter code for the panel.'  'LABEL (metadata, string) is an extended label of the panel.'  'NOTES (metadata, string) are some specific notes about the panel.'  'WAITBAR (gui, logical) detemines whether to show the waitbar.'  'H_WAITBAR (evanescent, handle) is the waitbar handle.'  'DRAW (query, logical) draws the panel [accepts uipanel Name-Value pairs].'  'DRAWN (query, logical) returns whether the panel has been drawn.'  'PARENT (gui, item) is the panel parent.'  'BKGCOLOR (figure, color) is the panel background color.'  'H (evanescent, handle) is the panel handle.'  'SHOW (query, logical) shows the figure containing the panel.'  'HIDE (query, logical) hides the figure containing the panel.'  'DELETE (query, logical) resets the handles when the panel is deleted.'  'CLOSE (query, logical) closes the figure containing the panel.' };
		...
		function prop_settings = getPropSettings(pointer)
			...
			switch prop %CET: Computational Efficiency Trick
				case 7 % Panel.WAITBAR
					prop_settings = Format.getFormatSettings(4);
				case 8 % Panel.H_WAITBAR
					prop_settings = Format.getFormatSettings(18);
				case 9 % Panel.DRAW
					prop_settings = Format.getFormatSettings(4);
				case 10 % Panel.DRAWN
					prop_settings = Format.getFormatSettings(4);
				case 11 % Panel.PARENT
					prop_settings = Format.getFormatSettings(8);
				case 12 % Panel.BKGCOLOR
				...
				case 15 % Panel.HIDE
					prop_settings = Format.getFormatSettings(4);
				case 16 % Panel.DELETE
					prop_settings = Format.getFormatSettings(4);
				case 17 % Panel.CLOSE
					prop_settings = Format.getFormatSettings(4);
				case 3 % Panel.TEMPLATE
					prop_settings = 'Panel';
				otherwise
		...
		function prop_default = getPropDefault(pointer)
			...
			switch prop %CET: Computational Efficiency Trick
				...
				case 1 % Panel.NAME
					prop_default = 'Panel';
				case 2 % Panel.DESCRIPTION
					prop_default = 'Panel is the basic element to manage graphical representations of elements. It is an empty graphical panel. It is filled with the graphical content representing an element (PanelElement) or figure (PanelFig) or property (typically, PanelProp and derived classes). It must be placed within another container; for example, a figure (e.g., PanelElement into GUIElement, or PanelFig into GUIFig) or another panel (e.g., PanelProp into PanelElement).';
				case 3 % Panel.TEMPLATE
