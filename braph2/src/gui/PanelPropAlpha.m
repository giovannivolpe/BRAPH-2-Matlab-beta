classdef PanelPropAlpha < PanelProp
	...
	%  tostring - string with information about the panel property alpha
	%  disp - displays information about the panel property alpha
	%  tree - displays the tree of the panel property alpha
	%
	% PanelPropAlpha methods (miscellanea):
	%  getNoValue - returns a pointer to a persistent instance of NoValue
	%               Use it as Element.getNoValue()
	%  getCallback - returns the callback to a property
	%  isequal - determines whether two panel property alpha are equal (values, locked)
	%  getElementList - returns a list with all subelements
	%  copy - copies the panel property alpha
	%
	% PanelPropAlpha methods (save/load, Static):
	%  save - saves BRAPH2 panel property alpha as b2 file
	%  load - loads a BRAPH2 panel property alpha from a b2 file
	%
	% PanelPropAlpha method (JSON encode):
	%  encodeJSON - returns a JSON string encoding the panel property alpha
	%
	% PanelPropAlpha method (JSON decode, Static):
	%   decodeJSON - returns a JSON string encoding the panel property alpha
	%
	% PanelPropAlpha methods (inspection, Static):
	%  getClass - returns the class of the panel property alpha
	%  getSubclasses - returns all subclasses of PanelPropAlpha
	%  getProps - returns the property list of the panel property alpha
	%  getPropNumber - returns the property number of the panel property alpha
	...
	methods % constructor
		function pr = PanelPropAlpha(varargin)
			%PanelPropAlpha() creates a panel property alpha.
			%
	...
	methods (Static) % inspection
		function pr_class = getClass()
			...
			%  CLASS = PR.GETCLASS() returns the class of the panel property alpha PR.
		...
		function subclass_list = getSubclasses()
			%GETSUBCLASSES returns all subclasses of the panel property alpha.
			%
			% LIST = PanelPropAlpha.GETSUBCLASSES() returns all subclasses of 'PanelPropAlpha'.
			%
			% Alternative forms to call this method are:
			%  LIST = PR.GETSUBCLASSES() returns all subclasses of the panel property alpha PR.
		...
		function prop_list = getProps(category)
			%GETPROPS returns the property list of panel property alpha.
			%
			% PROPS = PanelPropAlpha.GETPROPS() returns the property list of panel property alpha
			%  as a row vector.
			%
			...
			%
			% Alternative forms to call this method are:
			%  PROPS = PR.GETPROPS([CATEGORY]) returns the property list of the panel property alpha PR.
		...
		function prop_number = getPropNumber(varargin)
			%GETPROPNUMBER returns the property number of panel property alpha.
			%
			% N = PanelPropAlpha.GETPROPNUMBER() returns the property number of panel property alpha.
			%
			% N = PanelPropAlpha.GETPROPNUMBER(CATEGORY) returns the property number of panel property alpha
			%  of category CATEGORY
			%
			% Alternative forms to call this method are:
			%  N = PR.GETPROPNUMBER([CATEGORY]) returns the property number of the panel property alpha PR.
		...
		function check_out = existsProp(prop)
			%EXISTSPROP checks whether property exists in panel property alpha/error.
			%
		...
		function check_out = existsTag(tag)
			%EXISTSTAG checks whether tag exists in panel property alpha/error.
			%
		...
		function prop_default = getPropDefault(pointer)
			...
			switch prop
				...
				case PanelPropAlpha.DESCRIPTION
					prop_default = 'PanelPropAlpha plots the panel for a ALPHA property with a numeric edit field and a slider. It works for all categories.';
				case PanelPropAlpha.TEMPLATE
