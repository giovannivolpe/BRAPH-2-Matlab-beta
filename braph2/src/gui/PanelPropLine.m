classdef PanelPropLine < PanelProp
	...
	%  tostring - string with information about the panel property line
	%  disp - displays information about the panel property line
	%  tree - displays the tree of the panel property line
	%
	% PanelPropLine methods (miscellanea):
	%  getNoValue - returns a pointer to a persistent instance of NoValue
	%               Use it as Element.getNoValue()
	%  getCallback - returns the callback to a property
	%  isequal - determines whether two panel property line are equal (values, locked)
	%  getElementList - returns a list with all subelements
	%  copy - copies the panel property line
	%
	% PanelPropLine methods (save/load, Static):
	%  save - saves BRAPH2 panel property line as b2 file
	%  load - loads a BRAPH2 panel property line from a b2 file
	%
	% PanelPropLine method (JSON encode):
	%  encodeJSON - returns a JSON string encoding the panel property line
	%
	% PanelPropLine method (JSON decode, Static):
	%   decodeJSON - returns a JSON string encoding the panel property line
	%
	% PanelPropLine methods (inspection, Static):
	%  getClass - returns the class of the panel property line
	%  getSubclasses - returns all subclasses of PanelPropLine
	%  getProps - returns the property list of the panel property line
	%  getPropNumber - returns the property number of the panel property line
	...
	methods % constructor
		function pr = PanelPropLine(varargin)
			%PanelPropLine() creates a panel property line.
			%
	...
	methods (Static) % inspection
		function pr_class = getClass()
			...
			%  CLASS = PR.GETCLASS() returns the class of the panel property line PR.
		...
		function subclass_list = getSubclasses()
			%GETSUBCLASSES returns all subclasses of the panel property line.
			%
			% LIST = PanelPropLine.GETSUBCLASSES() returns all subclasses of 'PanelPropLine'.
			%
			% Alternative forms to call this method are:
			%  LIST = PR.GETSUBCLASSES() returns all subclasses of the panel property line PR.
		...
		function prop_list = getProps(category)
			%GETPROPS returns the property list of panel property line.
			%
			% PROPS = PanelPropLine.GETPROPS() returns the property list of panel property line
			%  as a row vector.
			%
			...
			%
			% Alternative forms to call this method are:
			%  PROPS = PR.GETPROPS([CATEGORY]) returns the property list of the panel property line PR.
		...
		function prop_number = getPropNumber(varargin)
			%GETPROPNUMBER returns the property number of panel property line.
			%
			% N = PanelPropLine.GETPROPNUMBER() returns the property number of panel property line.
			%
			% N = PanelPropLine.GETPROPNUMBER(CATEGORY) returns the property number of panel property line
			%  of category CATEGORY
			%
			% Alternative forms to call this method are:
			%  N = PR.GETPROPNUMBER([CATEGORY]) returns the property number of the panel property line PR.
		...
		function check_out = existsProp(prop)
			%EXISTSPROP checks whether property exists in panel property line/error.
			%
		...
		function check_out = existsTag(tag)
			%EXISTSTAG checks whether tag exists in panel property line/error.
			%
		...
		function prop_default = getPropDefault(pointer)
			...
			switch prop
				...
				case PanelPropLine.DESCRIPTION
					prop_default = 'PanelPropLine plots the panel for an LINE property with a drop-down list. It works for all categories.';
				case PanelPropLine.TEMPLATE
