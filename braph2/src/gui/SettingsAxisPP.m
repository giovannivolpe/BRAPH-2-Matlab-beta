classdef SettingsAxisPP < SettingsPP
	...
	%  tostring - string with information about the settings axis panel
	%  disp - displays information about the settings axis panel
	%  tree - displays the tree of the settings axis panel
	%
	% SettingsAxisPP methods (miscellanea):
	%  getNoValue - returns a pointer to a persistent instance of NoValue
	%               Use it as Element.getNoValue()
	%  getCallback - returns the callback to a property
	%  isequal - determines whether two settings axis panel are equal (values, locked)
	%  getElementList - returns a list with all subelements
	%  copy - copies the settings axis panel
	%
	% SettingsAxisPP methods (save/load, Static):
	%  save - saves BRAPH2 settings axis panel as b2 file
	%  load - loads a BRAPH2 settings axis panel from a b2 file
	%
	% SettingsAxisPP method (JSON encode):
	%  encodeJSON - returns a JSON string encoding the settings axis panel
	%
	% SettingsAxisPP method (JSON decode, Static):
	%   decodeJSON - returns a JSON string encoding the settings axis panel
	%
	% SettingsAxisPP methods (inspection, Static):
	%  getClass - returns the class of the settings axis panel
	%  getSubclasses - returns all subclasses of SettingsAxisPP
	%  getProps - returns the property list of the settings axis panel
	%  getPropNumber - returns the property number of the settings axis panel
	...
	methods % constructor
		function pr = SettingsAxisPP(varargin)
			%SettingsAxisPP() creates a settings axis panel.
			%
	...
	methods (Static) % inspection
		function pr_class = getClass()
			...
			%  CLASS = PR.GETCLASS() returns the class of the settings axis panel PR.
		...
		function subclass_list = getSubclasses()
			%GETSUBCLASSES returns all subclasses of the settings axis panel.
			%
			% LIST = SettingsAxisPP.GETSUBCLASSES() returns all subclasses of 'SettingsAxisPP'.
			%
			% Alternative forms to call this method are:
			%  LIST = PR.GETSUBCLASSES() returns all subclasses of the settings axis panel PR.
		...
		function prop_list = getProps(category)
			%GETPROPS returns the property list of settings axis panel.
			%
			% PROPS = SettingsAxisPP.GETPROPS() returns the property list of settings axis panel
			%  as a row vector.
			%
			...
			%
			% Alternative forms to call this method are:
			%  PROPS = PR.GETPROPS([CATEGORY]) returns the property list of the settings axis panel PR.
		...
		function prop_number = getPropNumber(varargin)
			%GETPROPNUMBER returns the property number of settings axis panel.
			%
			% N = SettingsAxisPP.GETPROPNUMBER() returns the property number of settings axis panel.
			%
			% N = SettingsAxisPP.GETPROPNUMBER(CATEGORY) returns the property number of settings axis panel
			%  of category CATEGORY
			%
			% Alternative forms to call this method are:
			%  N = PR.GETPROPNUMBER([CATEGORY]) returns the property number of the settings axis panel PR.
		...
		function check_out = existsProp(prop)
			%EXISTSPROP checks whether property exists in settings axis panel/error.
			%
		...
		function check_out = existsTag(tag)
			%EXISTSTAG checks whether tag exists in settings axis panel/error.
			%
		...
		function prop_description = getPropDescription(pointer)
			...
			switch prop
				...
				case SettingsAxisPP.NAME
					prop_description = 'NAME (constant, string) is the name of the settings axis panel.';
				case SettingsAxisPP.DESCRIPTION
					prop_description = 'DESCRIPTION (constant, string) is the description of the settings axis panel.';
				case SettingsAxisPP.TEMPLATE
					prop_description = 'TEMPLATE (parameter, item) is the template of the settings axis panel.';
				case SettingsAxisPP.ID
					prop_description = 'ID (data, string) is a few-letter code for the settings axis panel.';
				case SettingsAxisPP.LABEL
					prop_description = 'LABEL (metadata, string) is an extended label of the settings axis panel.';
				case SettingsAxisPP.NOTES
					prop_description = 'NOTES (metadata, string) are some specific notes about the settings axis panel.';
		...
		function prop_default = getPropDefault(pointer)
			...
			switch prop
				...
				case SettingsAxisPP.DESCRIPTION
					prop_default = 'SettingsAxisPP plots the panel for axis settings including a button for the axis color and checkboxes for axis on/off, grid on/off, equal on/off, and tight on/off. It works for all categories.';
				case SettingsAxisPP.TEMPLATE
