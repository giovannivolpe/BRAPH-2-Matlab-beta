classdef SettingsTextPP < SettingsPP
	...
	%  tostring - string with information about the settings text panel
	%  disp - displays information about the settings text panel
	%  tree - displays the tree of the settings text panel
	%
	% SettingsTextPP methods (miscellanea):
	%  getNoValue - returns a pointer to a persistent instance of NoValue
	%               Use it as Element.getNoValue()
	%  getCallback - returns the callback to a property
	%  isequal - determines whether two settings text panel are equal (values, locked)
	%  getElementList - returns a list with all subelements
	%  copy - copies the settings text panel
	%
	% SettingsTextPP methods (save/load, Static):
	%  save - saves BRAPH2 settings text panel as b2 file
	%  load - loads a BRAPH2 settings text panel from a b2 file
	%
	% SettingsTextPP method (JSON encode):
	%  encodeJSON - returns a JSON string encoding the settings text panel
	%
	% SettingsTextPP method (JSON decode, Static):
	%   decodeJSON - returns a JSON string encoding the settings text panel
	%
	% SettingsTextPP methods (inspection, Static):
	%  getClass - returns the class of the settings text panel
	%  getSubclasses - returns all subclasses of SettingsTextPP
	%  getProps - returns the property list of the settings text panel
	%  getPropNumber - returns the property number of the settings text panel
	...
	methods % constructor
		function pr = SettingsTextPP(varargin)
			%SettingsTextPP() creates a settings text panel.
			%
	...
	methods (Static) % inspection
		function pr_class = getClass()
			...
			%  CLASS = PR.GETCLASS() returns the class of the settings text panel PR.
		...
		function subclass_list = getSubclasses()
			%GETSUBCLASSES returns all subclasses of the settings text panel.
			%
			% LIST = SettingsTextPP.GETSUBCLASSES() returns all subclasses of 'SettingsTextPP'.
			%
			% Alternative forms to call this method are:
			%  LIST = PR.GETSUBCLASSES() returns all subclasses of the settings text panel PR.
		...
		function prop_list = getProps(category)
			...
			%
			% PROPS = SettingsTextPP.GETPROPS() returns the property list of settings text panel
			%  as a row vector.
			%
			...
			%
			% Alternative forms to call this method are:
			%  PROPS = PR.GETPROPS([CATEGORY]) returns the property list of the settings text panel PR.
		...
		function prop_number = getPropNumber(varargin)
			%GETPROPNUMBER returns the property number of settings text panel.
			%
			% N = SettingsTextPP.GETPROPNUMBER() returns the property number of settings text panel.
			%
			% N = SettingsTextPP.GETPROPNUMBER(CATEGORY) returns the property number of settings text panel
			%  of category CATEGORY
			%
			% Alternative forms to call this method are:
			%  N = PR.GETPROPNUMBER([CATEGORY]) returns the property number of the settings text panel PR.
		...
		function check_out = existsTag(tag)
			%EXISTSTAG checks whether tag exists in settings text panel/error.
			%
