classdef OrdMxWU < Graph
	...
	%  <strong>1</strong> <strong>NAME</strong> 	NAME (constant, string) is the name of the ordinal multiplex weighted undirected graph.
	%  <strong>2</strong> <strong>DESCRIPTION</strong> 	DESCRIPTION (constant, string) is the description of the ordinal multiplex weighted undirected graph.
	%  <strong>3</strong> <strong>TEMPLATE</strong> 	TEMPLATE (parameter, item) is the template of the ordinal multiplex weighted undirected graph.
	%  <strong>4</strong> <strong>ID</strong> 	ID (data, string) is a few-letter code for the ordinal multiplex weighted undirected graph.
	%  <strong>5</strong> <strong>LABEL</strong> 	LABEL (metadata, string) is an extended label of the ordinal multiplex weighted undirected graph.
	%  <strong>6</strong> <strong>NOTES</strong> 	NOTES (metadata, string) are some specific notes about the ordinal multiplex weighted undirected graph.
	%  <strong>7</strong> <strong>GRAPH_TYPE</strong> 	GRAPH_TYPE (constant, scalar) returns the graph type Graph.ORDERED_MULTIPLEX.
	...
	methods (Static) % inspection
		...
		function prop_category = getPropCategory(pointer)
			...
			%CET: Computational Efficiency Trick
			ordmxwu_category_list = { 1  1  3  4  2  2  1  6  6  6  6  2  6  2  6  2  6  3  3  5  6  5  5  5  5  1  6  9  9  4  3  3  3  3  3  6 };
		...
		function prop_format = getPropFormat(pointer)
			...
			%CET: Computational Efficiency Trick
			ordmxwu_format_list = { 2  2  8  2  2  2  11  15  15  15  11  12  12  3  3  3  3  4  11  16  4  12  11  12  10  7  8  8  8  16  5  5  5  11  11  16 };
		...
		function prop_settings = getPropSettings(pointer)
			...
			switch prop %CET: Computational Efficiency Trick
				...
				case 31 % OrdMxWU.SYMMETRIZE_RULE
					prop_settings = {'max', 'sum', 'average', 'min'};
				case 32 % OrdMxWU.SEMIPOSITIVIZE_RULE
					prop_settings = {'zero', 'absolute'};
				case 33 % OrdMxWU.STANDARDIZE_RULE
					prop_settings = {'threshold' 'range'};
				case 34 % OrdMxWU.ATTEMPTSPEREDGE
					prop_settings = Format.getFormatSettings(11);
				case 35 % OrdMxWU.NUMBEROFWEIGHTS
					prop_settings = Format.getFormatSettings(11);
				case 36 % OrdMxWU.RANDOMIZATION
					prop_settings = Format.getFormatSettings(16);
				case 3 % OrdMxWU.TEMPLATE
		...
		function prop_default = getPropDefault(pointer)
			...
			switch prop %CET: Computational Efficiency Trick
				...
				case 5 % OrdMxWU.LABEL
					prop_default = 'OrdMxWU label';
				case 6 % OrdMxWU.NOTES
					prop_default = 'OrdMxWU notes';
				case 7 % OrdMxWU.GRAPH_TYPE
					prop_default = 3;
				case 26 % OrdMxWU.COMPATIBLE_MEASURES
					prop_default = { 'Degree'  'DegreeAv'  'Diameter'  'Distance'  'Eccentricity'  'EccentricityAv'  'Flexibility'  'FlexibilityAv'  'GlobalEfficiency'  'GlobalEfficiencyAv'  'MultiRC'  'MultilayerCommunity'  'MultilayerM'  'MultiplexCP'  'MultiplexCl'  'MultiplexClAv'  'MultiplexKCor'  'MultiplexKCorC'  'MultiplexP'  'MultiplexPAv'  'MultiplexRCDeg'  'MultiplexRCS'  'MultiplexT'  'OverlappingDeg'  'OverlappingDegAv'  'OverlappingS'  'OverlappingSAv'  'RCDeg'  'Radius'  'Strength'  'StrengthAv'  'Triangles'  'WeightedEdgeOvlp'  'WeightedMxP'  'WeightedMxPAv' };
				otherwise
	...
	methods (Access=protected) % calculate value
		function value = calculateValue(g, prop, varargin)
			...
			switch prop
				...
				case 8 % OrdMxWU.CONNECTIVITY_TYPE
					if isempty(varargin)
					    layernumber = 1;
					else
				...
				case 9 % OrdMxWU.DIRECTIONALITY_TYPE
					if isempty(varargin)
					    layernumber = 1;
					else
					    layernumber = varargin{1};
				...
				case 10 % OrdMxWU.SELFCONNECTIVITY_TYPE
					if isempty(varargin)
					    layernumber = 1;
					else
					    layernumber = varargin{1};
				...
				case 11 % OrdMxWU.NEGATIVITY_TYPE
					if isempty(varargin)
					    layernumber = 1;
					else
					    layernumber = varargin{1};
