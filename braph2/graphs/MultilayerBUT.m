classdef MultilayerBUT < MultilayerWU
	%MultilayerBUT is a multilayer binary undirected with fixed thresholds graph.
		...
		function prop_description = getPropDescription(pointer)
			...
			switch prop
				...
				case MultilayerBUT.NAME
					prop_description = 'NAME (constant, string) is the name of the multilayer undirected with fixed thresholds graph.';
				case MultilayerBUT.DESCRIPTION
					prop_description = 'DESCRIPTION (constant, string) is the description of the multilayer undirected with fixed thresholds graph.';
				case MultilayerBUT.TEMPLATE
					prop_description = 'TEMPLATE (parameter, item) is the template of the multilayer binarized at different thresholds graph.';
				case MultilayerBUT.ID
					prop_description = 'ID (data, string) is a few-letter code for the multilayer binary  undirected with fixed thresholds graph.';
				case MultilayerBUT.LABEL
					prop_description = 'LABEL (metadata, string) is an extended label of the multilayer binary  undirected with fixed thresholds graph.';
				case MultilayerBUT.NOTES
					prop_description = 'NOTES (metadata, string) are some specific notes about the multilayer binary undirected with fixed thresholds graph.';
				...
				case MultilayerBUT.RANDOMIZATION
					prop_description = 'RANDOMIZATION (query, cell) is the attempts to rewire each edge.';
				case MultilayerBUT.SUBGRAPH
					prop_description = 'SUBGRAPH (query, item) returns a subgraph of original graph';
				otherwise
	...
	methods (Access=protected) % calculate value
		function value = calculateValue(g, prop, varargin)
			...
			switch prop
				...
				case MultilayerBUT.SUBGRAPH % __MultilayerBUT.SUBGRAPH__
					B = g.get('B');
					L = g.get('LAYERNUMBER');
					if isempty(varargin)
					    value = g;
