classdef MultilayerWU < Graph
	...
	%  <strong>1</strong> <strong>NAME</strong> 	NAME (constant, string) is the name of the multilayer weighted undirected graph.
	%  <strong>2</strong> <strong>DESCRIPTION</strong> 	DESCRIPTION (constant, string) is the description of the multilayer weighted undirected graph.
	%  <strong>3</strong> <strong>TEMPLATE</strong> 	TEMPLATE (parameter, item) is the template of the multilayer weighted undirected graph.
	%  <strong>4</strong> <strong>ID</strong> 	ID (data, string) is a few-letter code for the multilayer weighted undirected graph.
	%  <strong>5</strong> <strong>LABEL</strong> 	LABEL (metadata, string) is an extended label of the multilayer weighted undirected graph.
	%  <strong>6</strong> <strong>NOTES</strong> 	NOTES (metadata, string) are some specific notes about the multilayer weighted undirected graph.
	...
	%  <strong>20</strong> <strong>A</strong> 	A (result, cell) is the cell containing the weighted supra-adjacency matrix of the multilayer weighted undirected graph.
	...
	methods (Static) % inspection
		...
		function tag = getPropTag(pointer)
			...
			if ischar(pointer)
				...
				%CET: Computational Efficiency Trick
				multilayerwu_tag_list = { 'NAME'  'DESCRIPTION'  'TEMPLATE'  'ID'  'LABEL'  'NOTES'  'GRAPH_TYPE'  'CONNECTIVITY_TYPE'  'DIRECTIONALITY_TYPE'  'SELFCONNECTIVITY_TYPE'  'NEGATIVITY_TYPE'  'LAYERTICKS'  'ALAYERTICKS'  'LAYERLABELS'  'ALAYERLABELS'  'NODELABELS'  'ANODELABELS'  'RANDOMIZE'  'RANDOM_SEED'  'A'  'A_CHECK'  'NODENUMBER'  'LAYERNUMBER'  'PARTITIONS'  'M_DICT'  'COMPATIBLE_MEASURES'  'MEASURE'  'PFGA'  'PFGH'  'B'  'SYMMETRIZE_RULE'  'SEMIPOSITIVIZE_RULE'  'STANDARDIZE_RULE'  'ATTEMPTSPEREDGE'  'NUMBEROFWEIGHTS'  'RANDOMIZATION' };
		...
		function prop_category = getPropCategory(pointer)
			...
			%CET: Computational Efficiency Trick
			multilayerwu_category_list = { 1  1  3  4  2  2  1  6  6  6  6  2  6  2  6  2  6  3  3  5  6  5  5  5  5  1  6  9  9  4  3  3  3  3  3  6 };
		...
		function prop_format = getPropFormat(pointer)
			...
			%CET: Computational Efficiency Trick
			multilayerwu_format_list = { 2  2  8  2  2  2  11  15  15  15  11  12  12  3  3  3  3  4  11  16  4  12  11  12  10  7  8  8  8  16  5  5  5  11  11  16 };
		...
		function prop_settings = getPropSettings(pointer)
			...
			switch prop %CET: Computational Efficiency Trick
				case 30 % MultilayerWU.B
					prop_settings = Format.getFormatSettings(16);
				case 31 % MultilayerWU.SYMMETRIZE_RULE
					prop_settings = {'max', 'sum', 'average', 'min'};
				case 32 % MultilayerWU.SEMIPOSITIVIZE_RULE
					prop_settings = {'zero', 'absolute'};
				case 33 % MultilayerWU.STANDARDIZE_RULE
					prop_settings = {'threshold' 'range'};
				case 34 % MultilayerWU.ATTEMPTSPEREDGE
					prop_settings = Format.getFormatSettings(11);
				case 35 % MultilayerWU.NUMBEROFWEIGHTS
					prop_settings = Format.getFormatSettings(11);
				case 36 % MultilayerWU.RANDOMIZATION
					prop_settings = Format.getFormatSettings(16);
				case 3 % MultilayerWU.TEMPLATE
		...
		function prop_default = getPropDefault(pointer)
			...
			switch prop %CET: Computational Efficiency Trick
				...
				case 1 % MultilayerWU.NAME
					prop_default = 'MultilayerWU';
				case 2 % MultilayerWU.DESCRIPTION
				...
				case 4 % MultilayerWU.ID
					prop_default = 'MultilayerWU ID';
				case 5 % MultilayerWU.LABEL
					prop_default = 'MultilayerWU label';
				case 6 % MultilayerWU.NOTES
					prop_default = 'MultilayerWU notes';
				case 7 % MultilayerWU.GRAPH_TYPE
					prop_default = 6;
				case 26 % MultilayerWU.COMPATIBLE_MEASURES
					prop_default = { 'Assortativity'  'BetweennessCentrality'  'Degree'  'DegreeAv'  'Distance'  'Flexibility'  'FlexibilityAv'  'GlobalEfficiency'  'GlobalEfficiencyAv'  'MultilayerCommunity'  'MultilayerM'  'OverlappingDeg'  'OverlappingDegAv'  'OverlappingS'  'OverlappingSAv'  'PathLength'  'PathLengthAv'  'SmallWorldness'  'Strength'  'StrengthAv'  'Transitivity'  'WeightedEdgeOvlp' };
				otherwise
	...
	methods (Access=protected) % calculate value
		function value = calculateValue(g, prop, varargin)
			...
			switch prop
				...
				case 8 % MultilayerWU.CONNECTIVITY_TYPE
					if isempty(varargin)
					    layernumber = 1;
					else
				...
				case 9 % MultilayerWU.DIRECTIONALITY_TYPE
					if isempty(varargin)
					    layernumber = 1;
					else
					    layernumber = varargin{1};
				...
				case 10 % MultilayerWU.SELFCONNECTIVITY_TYPE
					if isempty(varargin)
					    layernumber = 1;
					else
					    layernumber = varargin{1};
				...
				case 11 % MultilayerWU.NEGATIVITY_TYPE
					if isempty(varargin)
					    layernumber = 1;
					else
					    layernumber = varargin{1};
				...
				case 15 % MultilayerWU.ALAYERLABELS
