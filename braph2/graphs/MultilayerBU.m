classdef MultilayerBU < Graph
	...
	%  <strong>1</strong> <strong>NAME</strong> 	NAME (constant, string) is the name of the multilayer binary undirected graph.
	%  <strong>2</strong> <strong>DESCRIPTION</strong> 	DESCRIPTION (constant, string) is the description of the multilayer binary undirected graph.
	%  <strong>3</strong> <strong>TEMPLATE</strong> 	TEMPLATE (parameter, item) is the template of the multilayer binary undirected graph.
	%  <strong>4</strong> <strong>ID</strong> 	ID (data, string) is a few-letter code for the multilayer binary undirected graph.
	%  <strong>5</strong> <strong>LABEL</strong> 	LABEL (metadata, string) is an extended label of the multilayer binary undirected graph.
	%  <strong>6</strong> <strong>NOTES</strong> 	NOTES (metadata, string) are some specific notes about the multilayer binary undirected graph.
	%  <strong>7</strong> <strong>GRAPH_TYPE</strong> 	GRAPH_TYPE (constant, scalar) returns the graph type Graph.MULTILAYER.
	...
	%  <strong>20</strong> <strong>A</strong> 	A (result, cell) is the cell containing the binary supra-adjacency matrix of the multilayer binary undirected graph.
	...
	methods (Static) % inspection
		...
		function prop_list = getProps(category)
			...
			switch category
				...
				case 3 % Category.PARAMETER
					prop_list = [3 18 19 31 32 33];
				case 4 % Category.DATA
					prop_list = [4 30];
				case 5 % Category.RESULT
					prop_list = [20 22 23 24 25];
				case 6 % Category.QUERY
					prop_list = [8 9 10 11 13 15 17 21 27 34];
				case 9 % Category.GUI
					prop_list = [28 29];
				otherwise
					prop_list = [];
		...
		function tag = getPropTag(pointer)
			...
			if ischar(pointer)
				...
				%CET: Computational Efficiency Trick
				multilayerbu_tag_list = { 'NAME'  'DESCRIPTION'  'TEMPLATE'  'ID'  'LABEL'  'NOTES'  'GRAPH_TYPE'  'CONNECTIVITY_TYPE'  'DIRECTIONALITY_TYPE'  'SELFCONNECTIVITY_TYPE'  'NEGATIVITY_TYPE'  'LAYERTICKS'  'ALAYERTICKS'  'LAYERLABELS'  'ALAYERLABELS'  'NODELABELS'  'ANODELABELS'  'RANDOMIZE'  'RANDOM_SEED'  'A'  'A_CHECK'  'NODENUMBER'  'LAYERNUMBER'  'PARTITIONS'  'M_DICT'  'COMPATIBLE_MEASURES'  'MEASURE'  'PFGA'  'PFGH'  'B'  'SYMMETRIZE_RULE'  'SEMIPOSITIVIZE_RULE'  'ATTEMPTSPEREDGE'  'RANDOMIZATION' };
		...
		function prop_category = getPropCategory(pointer)
			...
			%CET: Computational Efficiency Trick
			multilayerbu_category_list = { 1  1  3  4  2  2  1  6  6  6  6  2  6  2  6  2  6  3  3  5  6  5  5  5  5  1  6  9  9  4  3  3  3  6 };
		...
		function prop_format = getPropFormat(pointer)
			...
			%CET: Computational Efficiency Trick
			multilayerbu_format_list = { 2  2  8  2  2  2  11  15  15  15  11  12  12  3  3  3  3  4  11  16  4  12  11  12  10  7  8  8  8  16  5  5  11  16 };
		...
		function prop_settings = getPropSettings(pointer)
			...
			switch prop %CET: Computational Efficiency Trick
				case 30 % MultilayerBU.B
					prop_settings = Format.getFormatSettings(16);
				case 31 % MultilayerBU.SYMMETRIZE_RULE
					prop_settings = {'max', 'sum', 'average', 'min'};
				case 32 % MultilayerBU.SEMIPOSITIVIZE_RULE
					prop_settings = {'zero', 'absolute'};
				case 33 % MultilayerBU.ATTEMPTSPEREDGE
					prop_settings = Format.getFormatSettings(11);
				case 34 % MultilayerBU.RANDOMIZATION
					prop_settings = Format.getFormatSettings(16);
				case 3 % MultilayerBU.TEMPLATE
		...
		function prop_default = getPropDefault(pointer)
			...
			switch prop %CET: Computational Efficiency Trick
				...
				case 1 % MultilayerBU.NAME
					prop_default = 'MultilayerBU';
				case 2 % MultilayerBU.DESCRIPTION
				...
				case 4 % MultilayerBU.ID
					prop_default = 'MultilayerBU ID';
				case 5 % MultilayerBU.LABEL
					prop_default = 'MultilayerBU label';
				case 6 % MultilayerBU.NOTES
					prop_default = 'MultilayerBU notes';
				case 7 % MultilayerBU.GRAPH_TYPE
					prop_default = 6;
				case 26 % MultilayerBU.COMPATIBLE_MEASURES
					prop_default = { 'Degree'  'DegreeAv'  'DegreeOverlap'  'DegreeOverlapAv'  'Distance'  'Flexibility'  'FlexibilityAv'  'GlobalEfficiency'  'GlobalEfficiencyAv'  'MultilayerCommunity'  'MultilayerM' 'OverlappingDeg'  'OverlappingDegAv'};
				otherwise
	...
	methods (Access=protected) % calculate value
		function value = calculateValue(g, prop, varargin)
			...
			switch prop
				...
				case 8 % MultilayerBU.CONNECTIVITY_TYPE
					if isempty(varargin)
					    layernumber = 1;
					else
				...
				case 9 % MultilayerBU.DIRECTIONALITY_TYPE
					if isempty(varargin)
					    layernumber = 1;
					else
					    layernumber = varargin{1};
				...
				case 10 % MultilayerBU.SELFCONNECTIVITY_TYPE
					if isempty(varargin)
					    layernumber = 1;
					else
					    layernumber = varargin{1};
				...
				case 11 % MultilayerBU.NEGATIVITY_TYPE
					if isempty(varargin)
					    layernumber = 1;
					else
					    layernumber = varargin{1};
				...
				case 15 % MultilayerBU.ALAYERLABELS
