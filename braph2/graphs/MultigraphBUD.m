classdef MultigraphBUD < GraphWU
	...
	%  <strong>1</strong> <strong>NAME</strong> 	NAME (constant, string) is the name of the binary undirected multigraph with fixed densities.
	%  <strong>2</strong> <strong>DESCRIPTION</strong> 	DESCRIPTION (constant, string) is the description of the binary undirected multigraph with fixed densities.
	%  <strong>3</strong> <strong>TEMPLATE</strong> 	TEMPLATE (parameter, item) is the template of the binary undirected multigraph with fixed densities.
	%  <strong>4</strong> <strong>ID</strong> 	ID (data, string) is a few-letter code for the binary undirected multigraph with fixed densities.
	%  <strong>5</strong> <strong>LABEL</strong> 	LABEL (metadata, string) is an extended label of the binary undirected multigraph with fixed densities.
	%  <strong>6</strong> <strong>NOTES</strong> 	NOTES (metadata, string) are some specific notes about the binary undirected multigraph with fixed densities.
	%  <strong>7</strong> <strong>GRAPH_TYPE</strong> 	GRAPH_TYPE (constant, matrix) returns the graph type Graph.MULTIGRAPH.
	...
	%  <strong>10</strong> <strong>SELFCONNECTIVITY_TYPE</strong> 	SELFCONNECTIVITY_TYPE (query, smatrix) returns the self-connectivity type Graph.NONSELFCONNECTED * ones(layernumber).
	...
	%  <strong>20</strong> <strong>A</strong> 	A (result, cell) is the cell array with the symmetric binary adjacency matrices of the binary undirected multigraph.
	...
	%  <strong>24</strong> <strong>PARTITIONS</strong> 	PARTITIONS (result, rvector) returns the number of layers (1) for each partition (density) of the graph.
	...
	%  <strong>37</strong> <strong>DENSITIES</strong> 	DENSITIES (parameter, rvector) is the vector of densities.
	%
	...
	properties (Constant) % properties
		DENSITIES = 37; %CET: Computational Efficiency Trick
	...
	methods (Static) % inspection
		...
		function tag = getPropTag(pointer)
			...
			if ischar(pointer)
				...
				%CET: Computational Efficiency Trick
				multigraphbud_tag_list = { 'NAME'  'DESCRIPTION'  'TEMPLATE'  'ID'  'LABEL'  'NOTES'  'GRAPH_TYPE'  'CONNECTIVITY_TYPE'  'DIRECTIONALITY_TYPE'  'SELFCONNECTIVITY_TYPE'  'NEGATIVITY_TYPE'  'LAYERTICKS'  'ALAYERTICKS'  'LAYERLABELS'  'ALAYERLABELS'  'NODELABELS'  'ANODELABELS'  'RANDOMIZE'  'RANDOM_SEED'  'A'  'A_CHECK'  'NODENUMBER'  'LAYERNUMBER'  'PARTITIONS'  'M_DICT'  'COMPATIBLE_MEASURES'  'MEASURE'  'PFGA'  'PFGH'  'B'  'SYMMETRIZE_RULE'  'SEMIPOSITIVIZE_RULE'  'STANDARDIZE_RULE'  'ATTEMPTSPEREDGE'  'NUMBEROFWEIGHTS'  'RANDOMIZATION'  'DENSITIES' };
		...
		function prop_category = getPropCategory(pointer)
			...
			%CET: Computational Efficiency Trick
			multigraphbud_category_list = { 1  1  3  4  2  2  1  6  6  6  6  2  6  2  6  2  6  3  3  5  6  5  5  5  5  1  6  9  9  4  3  3  3  3  3  6  3 };
		...
		function prop_format = getPropFormat(pointer)
			...
			%CET: Computational Efficiency Trick
			multigraphbud_format_list = { 2  2  8  2  2  2  11  15  15  15  11  12  12  3  3  3  3  4  11  16  4  12  11  12  10  7  8  8  8  15  5  5  5  11  11  16  12 };
		...
		function prop_settings = getPropSettings(pointer)
			...
			switch prop %CET: Computational Efficiency Trick
				case 37 % MultigraphBUD.DENSITIES
					prop_settings = Format.getFormatSettings(12);
				case 3 % MultigraphBUD.TEMPLATE
		...
		function prop_default = getPropDefault(pointer)
			...
			switch prop %CET: Computational Efficiency Trick
				...
				case 1 % MultigraphBUD.NAME
					prop_default = 'MultigraphBUD';
				case 2 % MultigraphBUD.DESCRIPTION
	...
	methods (Access=protected) % calculate value
		function value = calculateValue(g, prop, varargin)
			...
			switch prop
				case 8 % MultigraphBUD.CONNECTIVITY_TYPE
					if isempty(varargin)
					    layernumber = 1;
					else
				...
				case 9 % MultigraphBUD.DIRECTIONALITY_TYPE
					if isempty(varargin)
					    layernumber = 1;
					else
					    layernumber = varargin{1};
				...
				case 10 % MultigraphBUD.SELFCONNECTIVITY_TYPE
					if isempty(varargin)
					    layernumber = 1;
					else
					    layernumber = varargin{1};
				...
				case 11 % MultigraphBUD.NEGATIVITY_TYPE
					if isempty(varargin)
					    layernumber = 1;
					else
					    layernumber = varargin{1};
				...
				case 24 % MultigraphBUD.PARTITIONS
				...
				case 15 % MultigraphBUD.ALAYERLABELS
				...
				case 36 % MultigraphBUD.RANDOMIZATION
	...
	methods % GUI
		function pr = getPanelProp(g, prop, varargin)
			...
			switch prop
				case 37 % MultigraphBUD.DENSITIES
					pr = PanelPropRVectorSmart('EL', g, 'PROP', 37, 'MAX', 100, 'MIN', 0, varargin{:});
